...
#define WAIT_EVENT_GSS_OPEN_SERVER 1234 
/*
...
static uint32_t PqGSSMaxPktSize;	/* Maximum size we can encrypt and fit the
...
be_gssapi_write(ClientSocket *client_sock , void *ptr, size_t len)
{
	...
	{
		perror( "GSSAPI caller failed to retransmit all data needing to be retried");
		return -1;
	}
	...
	while (bytes_to_encrypt || PqGSSSendLength)
	{
		...
		if (PqGSSSendLength)
		{
			...
			ret = secure_raw_write(client_sock, PqGSSSendBuffer + PqGSSSendNext, amount);
			if (ret <= 0)
				return ret;
		...
		{
			pg_GSS_error(("GSSAPI wrap error"), major, minor);
			errno = ECONNRESET;
			return -1;
		}
		if (conf_state == 0)
		{
			fprintf(stderr, "outgoing GSSAPI message would not use confidentiality\n");
			return -1;
		}
		if (output.length > PQ_GSS_SEND_BUFFER_SIZE - sizeof(uint32))
		{
							
			printf("server tried to send oversize GSSAPI packet (%zu > %zu)",
...
be_gssapi_read(ClientSocket *client_sock , void *ptr, size_t len)
{
	...
	while (bytes_returned < len)
	{
		...
		{
			ret = secure_raw_read(client_sock, PqGSSRecvBuffer + PqGSSRecvLength,
		...
		{
			printf("server tried to send oversize GSSAPI packet (%zu > %zu)", (size_t) input.length,
		...
		if (major != GSS_S_COMPLETE)
		{
			pg_GSS_error(("GSSAPI unwrap error"), major, minor);
			errno = ECONNRESET;
			return -1;
		}
		if (conf_state == 0)
		{
		printf("incoming GSSAPI message did not use confidentiality");
			return -1;
		}
 ...
 */
static ssize_t
read_or_wait(ClientSocket *client_sock, ssize_t len)
{
	ssize_t		ret;
	...
	{
		ret = secure_raw_read(client_sock, 
                            PqGSSRecvBuffer + total_read, 
                            len - total_read);
...
ssize_t secure_open_gssapi(ClientSocket *client_sock)
{
	...
	gss = (pg_gssinfo *) calloc(1, sizeof(pg_gssinfo));

	delegated_creds = GSS_C_NO_CREDENTIAL;
	gss->delegated_creds = false;
    ...
    /*
     * Use keytab from environment (KRB5_KTNAME) if available.
     * If unset, GSSAPI will use the default keytab.
     */
    const char *krb5_ktname = getenv("KRB5_KTNAME");
    if (krb5_ktname != NULL && krb5_ktname[0] != '\0') {
        if (setenv("KRB5_KTNAME", krb5_ktname, 1) != 0) {
            fprintf(stderr, "could not set KRB5_KTNAME environment\n");
        }
    }
	...
	while (true)
	{
		 ...
		 */
		ret = read_or_wait(client_sock, sizeof(uint32));
		if (ret < 0)
			return ret;
		...
		{
			fprintf(stderr, "oversize GSSAPI packet sent by the client (%zu > %d)\n", (size_t) input.length,
		 ...
		 * the context.
		 */
		ret = read_or_wait(client_sock, input.length);
		if (ret < 0)
			return ret;
		...
		major = gss_accept_sec_context(&minor, &gss->ctx,
									   GSS_C_NO_CREDENTIAL, &input,
									   GSS_C_NO_CHANNEL_BINDINGS,
									   &gss->name, NULL, &output, NULL,
									   NULL, pg_gss_accept_delegation ? &delegated_creds : NULL);

		if (GSS_ERROR(major))
		{
			pg_GSS_error(("could not accept GSSAPI security context"),
						 major, minor);
			gss_release_buffer(&minor, &output);
			return -1;
		}
		...
		if (output.length > 0)
		{
			...
			if (output.length > PQ_GSS_SEND_BUFFER_SIZE - sizeof(uint32))
			{
				fprintf(stderr, "server tried to send oversize GSSAPI packet (%zu > %zu)\n", (size_t) output.length,
								PQ_GSS_SEND_BUFFER_SIZE - sizeof(uint32));
				gss_release_buffer(&minor, &output);
				return -1;
			}
			...
			{
				ret = secure_raw_write(client_sock, PqGSSSendBuffer + PqGSSSendNext,
	 ...
	 */
	major = gss_wrap_size_limit(&minor, gss->ctx, 1, GSS_C_QOP_DEFAULT,
								PQ_GSS_SEND_BUFFER_SIZE - sizeof(uint32),
								&PqGSSMaxPktSize);
