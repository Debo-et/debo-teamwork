 ...
 * with malloc() (falling back to malloc in frontend code).
 *
 *
 *-------------------------------------------------------------------------
 */
...
appendStringInfoVA(StringInfo str, const char *fmt, va_list args)
{
	...
	nprinted = (size_t)vsnprintf(str->data + str->len, (size_t) avail, fmt, args);
 ...
 * to store data in it.
 *
 * NB: In the backend, because we use malloc() to enlarge the buffer, the
...
enlargeStringInfo(StringInfo str, int needed)
{
	...
	{
#ifndef FRONTEND
				 printf("string buffer exceeds maximum allowed length ");
#else
		fprintf(stderr,
 ...
 * This must only be called on malloc'd StringInfos.
 */
void
