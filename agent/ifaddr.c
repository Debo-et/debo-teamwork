 ...
 */
int
range_sockaddr(const struct sockaddr_storage *addr,
    ...
    case AF_INET:
        {
            ...
            if (bits > 0)
                ...
                maskl = 0;
            mask4.sin_addr.s_addr = hton32(maskl);
 ...
 */
static void
run_ifaddr_callback(DbIfAddrCallback callback, void *cb_data,
 ...
 */
int
foreach_ifaddr(DbIfAddrCallback callback, void *cb_data)
{
    INTERFACE_INFO *ptr,
                   *ii = NULL;
...
foreach_ifaddr(DbIfAddrCallback callback, void *cb_data)
{
    ...
    while (n_buffer < 1024 * 100)
    {
         ...
         */
        if ((size_t)ifc.ifc_len < n_buffer - 1024)
            break;
    }
...
foreach_ifaddr(DbIfAddrCallback callback, void *cb_data)
{
    ...
    addr.sin_addr.s_addr = ntoh32(0x7f000001);
    memset(&mask, 0, sizeof(mask));
    sockaddr_cidr_mask(&mask, "8", AF_INET);
    run_ifaddr_callback(callback, cb_data,
    ...
    memset(&mask, 0, sizeof(mask));
    sockaddr_cidr_mask(&mask, "128", AF_INET6);
    run_ifaddr_callback(callback, cb_data,
