 ...
 */
ssize_t
pg_GSS_write(Conn *conn, const void *ptr, size_t len)
{
    OM_uint32	major,
                minor;
    ...
    while (bytes_to_encrypt || PqGSSSendLength)
    {
        ...
        if (PqGSSSendLength)
        {
            ...
            retval = secure_raw_write(conn, PqGSSSendBuffer + PqGSSSendNext, amount);
            if (retval <= 0)
                return retval;
        ...
        if (conf_state == 0)
        {
            db_append_conn_error(conn, "outgoing GSSAPI message would not use confidentiality");
            errno = EIO;		/* for lack of a better idea */
            goto cleanup;
        }
        ...
        {
            db_append_conn_error(conn, "client tried to send oversize GSSAPI packet (%zu > %zu)",
        ...
        netlen = db_hton32(output.length);
 ...
 */
ssize_t
pg_GSS_read(Conn *conn, void *ptr, size_t len)
{
    OM_uint32	major,
                minor;
    ...
    while (bytes_returned < len)
    {
        ...
        {
            ret = secure_raw_read(conn, PqGSSRecvBuffer + PqGSSRecvLength,
        ...
        input.length = db_ntoh32(*(uint32 *) PqGSSRecvBuffer);

        if (input.length > PQ_GSS_RECV_BUFFER_SIZE - sizeof(uint32))
        {
            db_append_conn_error(conn, "oversize GSSAPI packet sent by the server (%zu > %zu)",
                                 (size_t) input.length,
        ...
        if (conf_state == 0)
        {
            db_append_conn_error(conn, "incoming GSSAPI message did not use confidentiality");
            ret = -1;
            errno = EIO;		/* for lack of a better idea */
            goto cleanup;
        }
...
/*
 * Simple wrapper for reading from secure_raw_read.
 *
 * This takes the same arguments as secure_raw_read, plus an output parameter
 * to return the number of bytes read.  This handles if blocking would occur and
 * if we detect EOF on the connection.
 */
static PollingStatusType
gss_read(Conn *conn, void *recv_buffer, size_t length, ssize_t *ret)
{
    *ret = secure_raw_read(conn, recv_buffer, length);
    if (*ret < 0)
    {
...
pqsecure_open_gss(Conn *conn)
{
    ...
    if (PqGSSSendLength)
    {
        ...
        ret = secure_raw_write(conn, PqGSSSendBuffer + PqGSSSendNext, amount);
        if (ret < 0)
        {
            if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR)
    ...
    if (conn->gctx)
    {
        ...
        if (PqGSSRecvBuffer[0] == 'E')
        {
            ...
            appendExpBuffer(&conn->errorMessage, "%s\n", PqGSSRecvBuffer + 1);
