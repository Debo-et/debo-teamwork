...
Conn* startcommand(const char* host, const char* port) {
        Conn* conn = connect_to_debo(host, port);
        if (conn == NULL) {
            fprintf(stderr, "Error: Connection failed for step \n");
        }
return conn;
}

void endsendcommand(Conn* conn) {
    ...
    if (!conn) {
        fprintf(stderr, "Invalid connection pointer\n");
        return;
    }
...
void configure_target_remote_component(Component target, const char* host, const char* port) {
 ...
 //ZooKeeper Failover Controller settings
conn = startcommand(host, port); SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ha.failover-controller.active-standby-elector.zk.op.retries", "120", conn);endsendcommand(conn);

 //I/O properties
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "io.file.buffer.size", "131072", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "io.serializations", "org.apache.hadoop.io.serializer.WritableSerialization", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "io.compression.codecs", "org.apache.hadoop.io.compress.GzipCodec,org.apache.hadoop.io.compress.DefaultCodec,org.apache.hadoop.io.compress.SnappyCodec", conn);
endsendcommand(conn);

 //File system properties
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "fs.defaultFS", "hdfs:localhost:8020", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "fs.trash.interval", "360", conn);
endsendcommand(conn);

 //IPC properties
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ipc.client.idlethreshold", "8000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ipc.client.connection.maxidletime", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ipc.client.connect.max.retries", "50", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ipc.server.tcpnodelay", "true", conn);
endsendcommand(conn);

 //Web Interface Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.jobtracker.webinterface.trusted", "false", conn);
endsendcommand(conn);

 //Security properties
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.http.authentication.type", "simple", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.security.authentication", "simple", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.security.authorization", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.security.auth_to_local", "DEFAULT", conn);
endsendcommand(conn);

// Network topology
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "net.topology.script.file.name", "/etc/hadoop/conf/topology_script.py", conn);
endsendcommand(conn);

// Proxy user (empty value)
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.proxyuser", "", conn);
endsendcommand(conn);

// Cloud storage properties
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "fs.gs.path.encoding", "uri-path", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "fs.gs.working.dir", "/", conn);
endsendcommand(conn);

// Client protocol ACLs
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.client.protocol.acl", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.client.datanode.protocol.acl", "*", conn);
endsendcommand(conn);

 //DataNode protocol ACLs
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.datanode.protocol.acl", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.inter.datanode.protocol.acl", "*", conn);
endsendcommand(conn);

 //NameNode protocol ACLs
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.namenode.protocol.acl", "*", conn);
endsendcommand(conn);

 //JobTracker/TaskTracker protocol ACLs
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.inter.tracker.protocol.acl", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.job.client.protocol.acl", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.job.task.protocol.acl", "*", conn);
endsendcommand(conn);

 //Administrative protocol ACLs
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.admin.operations.protocol.acl", "hadoop", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.refresh.usertogroups.mappings.protocol.acl", "hadoop", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "security.refresh.policy.protocol.acl", "hadoop", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop_security_log_max_backup_size", "256", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop_security_log_number_of_backup_files", "20", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop_log_max_backup_size", "256", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop_log_number_of_backup_files", "10", conn);
endsendcommand(conn);

 //Root logger configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.rootLogger", "INFO,console,EventCounter", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.threshhold", "ALL", conn);
endsendcommand(conn);

 //Daily Rolling File Appender (DRFA)
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFA", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFA.File", "${hadoop.log.dir}/${hadoop.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFA.DatePattern", ".yyyy-MM-dd", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFA.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFA.layout.ConversionPattern", "%d{ISO8601} %p %c: %m%n", conn);
endsendcommand(conn);

 //Console Appender
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.console", "org.apache.log4j.ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.console.target", "System.err", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.console.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.console.layout.ConversionPattern", "%d{yy/MM/dd HH:mm:ss} %p %c{2}: %m%n", conn);
endsendcommand(conn);

 //TaskLog Appender (TLA)
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.TLA", "org.apache.hadoop.mapred.TaskLogAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.TLA.taskId", "${hadoop.tasklog.taskid}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.TLA.isCleanup", "${hadoop.tasklog.iscleanup}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.TLA.totalLogFileSize", "${hadoop.tasklog.totalLogFileSize}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.TLA.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.TLA.layout.ConversionPattern", "%d{ISO8601} %p %c: %m%n", conn);
endsendcommand(conn);

 //Security Audit Appender (DRFAS/RFAS)
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.category.SecurityLogger", "${hadoop.security.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFAS", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFAS.File", "${hadoop.log.dir}/${hadoop.security.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFAS.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFAS.layout.ConversionPattern", "%d{ISO8601} %p %c: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFAS.DatePattern", ".yyyy-MM-dd", conn);
endsendcommand(conn);

 //Rolling File Appender (RFA)
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RFA", "org.apache.log4j.RollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RFA.File", "${hadoop.log.dir}/${hadoop.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RFA.MaxFileSize", "${hadoop.security.log.maxfilesize}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RFA.MaxBackupIndex", "${hadoop.security.log.maxbackupindex}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RFA.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RFA.layout.ConversionPattern", "%d{ISO8601} %-5p %c{2} (%F:%M(%L)) - %m%n", conn);
endsendcommand(conn);

// HDFS Audit Logging
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.logger.org.apache.hadoop.hdfs.server.namenode.FSNamesystem.audit", "${hdfs.audit.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.additivity.org.apache.hadoop.hdfs.server.namenode.FSNamesystem.audit", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFAAUDIT", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFAAUDIT.File", "${hadoop.log.dir}/hdfs-audit.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFAAUDIT.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFAAUDIT.layout.ConversionPattern", "%d{ISO8601} %p %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.DRFAAUDIT.DatePattern", ".yyyy-MM-dd", conn);
endsendcommand(conn);

 //MapReduce Audit Logging
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.logger.org.apache.hadoop.mapred.AuditLogger", "${mapred.audit.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.additivity.org.apache.hadoop.mapred.AuditLogger", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.MRAUDIT", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.MRAUDIT.File", "${hadoop.log.dir}/mapred-audit.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.MRAUDIT.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.MRAUDIT.layout.ConversionPattern", "%d{ISO8601} %p %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.MRAUDIT.DatePattern", ".yyyy-MM-dd", conn);
endsendcommand(conn);

 //Custom Logging Levels
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.logger.org.apache.hadoop.metrics2", "${hadoop.metrics.log.level}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.logger.org.jets3t.service.impl.rest.httpclient.RestS3Service", "ERROR", conn);
endsendcommand(conn);

 //Special Appenders
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.NullAppender", "org.apache.log4j.varia.NullAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.EventCounter", "org.apache.hadoop.log.metrics.EventCounter", conn);
endsendcommand(conn);

 ///Warning Suppression
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.logger.org.apache.hadoop.conf.Configuration.deprecation", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.logger.org.apache.commons.beanutils", "WARN", conn);
endsendcommand(conn);

// RPC Service Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.rpc.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.rpc-address", "0.0.0.0:20010", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.rpc-bind-host", "0.0.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.handler.count", "10", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.handler.queue.size", "100", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.reader.count", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.reader.queue.size", "100", conn);
endsendcommand(conn);

 //Connection Pool Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.connection.creator.queue-size", "100", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.connection.pool-size", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.connection.min-active-ratio", "0.5f", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.connection.clean.ms", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.connection.pool.clean.ms", "60000", conn);
endsendcommand(conn);

 //Metrics and Monitoring
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.metrics.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.dn-report.time-out", "1000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.dn-report.cache-expire", "10s", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.metrics.class", "org.apache.hadoop.hdfs.server.federation.metrics.FederationRPCPerformanceMonitor", conn);
endsendcommand(conn);

 //Admin Service Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.admin.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.admin-address", "0.0.0.0:8111", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.admin-bind-host", "0.0.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.admin.handler.count", "1", conn);
endsendcommand(conn);

// Web Interface Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.http-address", "0.0.0.0:50071", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.http-bind-host", "0.0.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.https-address", "0.0.0.0:50072", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.https-bind-host", "0.0.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.http.enable", "true", conn);
endsendcommand(conn);

// Resolver Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.file.resolver.client.class", "org.apache.hadoop.hdfs.server.federation.resolver.MountTableResolver", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.namenode.resolver.client.class", "org.apache.hadoop.hdfs.server.federation.resolver.MembershipNamenodeResolver", conn);
endsendcommand(conn);

 //State Store Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.store.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.store.serializer", "org.apache.hadoop.hdfs.server.federation.store.driver.impl.StateStoreSerializerPBImpl", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.store.driver.class", "org.apache.hadoop.hdfs.server.federation.store.driver.impl.StateStoreZooKeeperImpl", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.store.connection.test", "60000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.cache.ttl", "1m", conn);
endsendcommand(conn);

 //Membership Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.store.membership.expiration", "300000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.store.membership.expiration.deletion", "-1", conn);
endsendcommand(conn);

 //Heartbeat Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.heartbeat.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.heartbeat.interval", "5000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.heartbeat-state.interval", "5s", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.namenode.heartbeat.enable", "true", conn);
endsendcommand(conn);

 //Router State Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.store.router.expiration", "5m", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.store.router.expiration.deletion", "-1", conn);
endsendcommand(conn);

 //Safe Mode Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.safemode.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.safemode.extension", "30s", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.safemode.expiration", "3m", conn);
endsendcommand(conn);

 //Monitoring Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.monitor.localnamenode.enable", "true", conn);
endsendcommand(conn);

// Mount Table Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.mount-table.max-cache-size", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.mount-table.cache.enable", "true", conn);
endsendcommand(conn);

 //Quota Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.quota.enable", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.quota-cache.update.interval", "60s", conn);
endsendcommand(conn);

 //Client Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.client.thread-size", "32", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.client.retry.max.attempts", "3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.client.reject.overload", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.client.allow-partial-listing", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.client.mount-status.time-out", "1s", conn);
endsendcommand(conn);

 //Connection Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.connect.max.retries.on.timeouts", "0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.connect.timeout", "2s", conn);
endsendcommand(conn);

 //Cache Update Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.mount-table.cache.update", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.mount-table.cache.update.timeout", "1m", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.mount-table.cache.update.client.max.time", "5m", conn);
endsendcommand(conn);

 //Security Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.federation.router.secret.manager.class", "org.apache.hadoop.hdfs.server.federation.router.security.token.ZKDelegationTokenSecretManagerImpl", conn);
endsendcommand(conn);

 //NameNode Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.name.dir", "/hadoop/hdfs/namenode", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.support.append", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.webhdfs.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.checkpoint.dir", "/hadoop/hdfs/namesecondary", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.checkpoint.edits.dir", "${dfs.namenode.checkpoint.dir}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.checkpoint.period", "21600", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.checkpoint.txns", "1000000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.handler.count", "100", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.http-address", "localhost:50070", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.https-address", "localhost:50470", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.rpc-address", "localhost:8020", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.secondary.http-address", "localhost:50090", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.safemode.threshold-pct", "0.999", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.name.dir.restore", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.accesstime.precision", "0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.avoid.read.stale.datanode", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.avoid.write.stale.datanode", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.write.stale.datanode.ratio", "1.0f", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.namenode.stale.datanode.interval", "30000", conn);
endsendcommand(conn);

 //DataNode Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.datanode.data.dir", "/hadoop/hdfs/data", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.datanode.failed.volumes.tolerated", "0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.datanode.address", "0.0.0.0:50010", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.datanode.http.address", "0.0.0.0:50075", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.datanode.https.address", "0.0.0.0:50475", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.datanode.ipc.address", "0.0.0.0:8010", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.datanode.du.reserved", "1073741824", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.datanode.data.dir.perm", "750", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.datanode.max.transfer.threads", "1024", conn);
endsendcommand(conn);

 //Block and Replication Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.blocksize", "134217728", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.replication", "3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.replication.max", "50", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.blockreport.initialDelay", "120", conn);
endsendcommand(conn);

// Heartbeat and Network Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.heartbeat.interval", "3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.https.port", "50470", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.http.policy", "HTTP_ONLY", conn);
endsendcommand(conn);

 //Permissions and Security
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "fs.permissions.umask-mode", "022", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.permissions.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.permissions.superusergroup", "hdfs", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.block.access.token.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.cluster.administrators", " hdfs", conn);
endsendcommand(conn);

 //JournalNode Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.journalnode.http-address", "0.0.0.0:8480", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.journalnode.https-address", "0.0.0.0:8481", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.journalnode.edits.dir", "/grid/0/hdfs/journal", conn);
endsendcommand(conn);

 //Short-Circuit Local Reads
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.client.read.shortcircuit", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.domain.socket.path", "/var/lib/hadoop-hdfs/dn_socket", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.client.read.shortcircuit.streams.cache.size", "4096", conn);
endsendcommand(conn);

 //Host Management
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.hosts.exclude", "/etc/hadoop/conf/dfs.exclude", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "manage.include.files", "false", conn);
endsendcommand(conn);

 //Balancing Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "dfs.datanode.balance.bandwidthPerSec", "6250000", conn);
endsendcommand(conn);

 //Audit Enablement Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.is.enabled", "true", conn);
endsendcommand(conn);

 //HDFS Audit Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.hdfs", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.hdfs.dir", "hdfs:NAMENODE_HOSTNAME:8020/ranger/audit", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.hdfs.batch.filespool.dir", "/var/log/hadoop/hdfs/audit/hdfs/spool", conn);
endsendcommand(conn);

 //Solr Audit Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.solr", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.solr.urls", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.solr.zookeepers", "NONE", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.solr.batch.filespool.dir", "/var/log/hadoop/hdfs/audit/solr/spool", conn);
endsendcommand(conn);

 //Additional Audit Settings
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.provider.summary.enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.hdfs.ambari.cluster.name", "{{cluster_name}}", conn);
endsendcommand(conn);

 //Basic Plugin Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "policy_user", "ambari-qa", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.rpc.protection", "authentication", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "common.name.for.certificate", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger-hdfs-plugin-enabled", "No", conn);
endsendcommand(conn);

 //Repository Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "REPOSITORY_CONFIG_USERNAME", "hadoop", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "REPOSITORY_CONFIG_PASSWORD", "hadoop", conn);
endsendcommand(conn);

 //External Admin Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "external_admin_username", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "external_admin_password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "external_ranger_admin_username", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "external_ranger_admin_password", "", conn);
endsendcommand(conn);

 //SSL Keystore Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.policymgr.clientssl.keystore", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.policymgr.clientssl.keystore.password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.policymgr.clientssl.keystore.credential.file", "jceks:file{{credential_file}}", conn);
endsendcommand(conn);

// SSL Truststore Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.policymgr.clientssl.truststore", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.policymgr.clientssl.truststore.password", "changeit", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.policymgr.clientssl.truststore.credential.file", "jceks:file{{credential_file}}", conn);
endsendcommand(conn);

// Ranger Service Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.hdfs.service.name", "{{repo_name}}", conn);
endsendcommand(conn);

 //Policy Source Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.hdfs.policy.source.impl", "org.apache.ranger.admin.client.RangerAdminRESTClient", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.hdfs.policy.rest.url", "{{policymgr_mgr_url}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.hdfs.policy.rest.ssl.config.file", "/etc/hadoop/conf/ranger-policymgr-ssl.xml", conn);
endsendcommand(conn);

// Policy Polling and Caching
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.hdfs.policy.pollIntervalMs", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.hdfs.policy.cache.dir", "/etc/ranger/{{repo_name}}/policycache", conn);
endsendcommand(conn);

 //Authorization Fallback
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.add-hadoop-authorization", "true", conn);
endsendcommand(conn);

 //Truststore Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.client.truststore.location", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.client.truststore.type", "jks", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.client.truststore.password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.client.truststore.reload.interval", "10000", conn);
endsendcommand(conn);

 //Keystore Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.client.keystore.type", "jks", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.client.keystore.location", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.client.keystore.password", "", conn);
endsendcommand(conn);

 //Truststore Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.server.truststore.location", "/etc/security/serverKeys/all.jks", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.server.truststore.type", "jks", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.server.truststore.password", "bigdata", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.server.truststore.reload.interval", "10000", conn);
endsendcommand(conn);

 //Keystore Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.server.keystore.type", "jks", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.server.keystore.location", "/etc/security/serverKeys/keystore.jks", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.server.keystore.password", "bigdata", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ssl.server.keystore.keypassword", "bigdata", conn);
endsendcommand(conn);


 //Update Hadoop MapReduce configuration parameters in mapred-site.xml
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.task.io.sort.mb", "358", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.map.sort.spill.percent", "0.7", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.task.io.sort.factor", "100", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.cluster.administrators", " hadoop", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.reduce.shuffle.parallelcopies", "30", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.map.speculative", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.reduce.speculative", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.job.reduce.slowstart.completedmaps", "0.05", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.job.counters.max", "130", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.reduce.shuffle.merge.percent", "0.66", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.reduce.shuffle.input.buffer.percent", "0.7", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.output.fileoutputformat.compress.type", "BLOCK", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.reduce.input.buffer.percent", "0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.map.output.compress", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.task.timeout", "300000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.map.memory.mb", "512", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.reduce.memory.mb", "1024", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.shuffle.port", "13562", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.jobhistory.intermediate-done-dir", "/mr-history/tmp", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.jobhistory.done-dir", "/mr-history/done", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.jobhistory.address", "localhost:10020", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.jobhistory.webapp.address", "localhost:19888", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.framework.name", "yarn", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.app.mapreduce.am.staging-dir", "/user", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.app.mapreduce.am.resource.mb", "512", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.app.mapreduce.am.command-opts", "-Xmx410m", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.app.mapreduce.am.admin-command-opts", "-server -XX:NewRatio=8 -Djava.net.preferIPv4Stack=true -Dhadoop.metrics.log.level=WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.app.mapreduce.am.log.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.admin.map.child.java.opts", "-server -XX:NewRatio=8 -Djava.net.preferIPv4Stack=true -Dhadoop.metrics.log.level=WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.admin.reduce.child.java.opts", "-server -XX:NewRatio=8 -Djava.net.preferIPv4Stack=true -Dhadoop.metrics.log.level=WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.application.classpath", "{{hadoop_conf_dir}},{{hadoop_home}}/*,{{hadoop_home}}/lib/*,{{hadoop_hdfs_home}}/*,{{hadoop_hdfs_home}}/lib/*,{{hadoop_yarn_home}}/*,{{hadoop_yarn_home}}/lib/*,{{hadoop_mapred_home}}/*,{{hadoop_mapred_home}}/lib/*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.am.max-attempts", "2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.map.java.opts", "-Xmx410m", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.reduce.java.opts", "-Xmx756m", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.map.log.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.reduce.log.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.admin.user.env", "LD_LIBRARY_PATH={{hadoop_home}}/lib/native", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.output.fileoutputformat.compress", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.jobhistory.http.policy", "HTTP_ONLY", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "mapreduce.job.queuename", "default", conn);
endsendcommand(conn);

 //Update YARN configuration parameters in yarn-site.xml
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.hostname", "localhost", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.resource-tracker.address", "localhost:8025", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.scheduler.address", "localhost:8030", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.address", "localhost:8050", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.admin.address", "localhost:8141", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.scheduler.class", "org.apache.hadoop.yarn.server.resourcemanager.scheduler.capacity.CapacityScheduler", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.minimum-allocation-mb", "512", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.maximum-allocation-mb", "5120", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.acl.enable", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.admin.acl", "yarn,yarn-ats", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.address", "0.0.0.0:45454", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.resource.memory-mb", "5120", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.application.classpath", "{{hadoop_conf_dir}},{{hadoop_home}}/*,{{hadoop_home}}/lib/*,{{hadoop_hdfs_home}}/*,{{hadoop_hdfs_home}}/lib/*,{{hadoop_yarn_home}}/*,{{hadoop_yarn_home}}/lib/*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.vmem-pmem-ratio", "2.1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.container-executor.class", "org.apache.hadoop.yarn.server.nodemanager.DefaultContainerExecutor", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.linux-container-executor.group", "hadoop", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.aux-services", "mapreduce_shuffle,{{timeline_collector}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.aux-services.mapreduce_shuffle.class", "org.apache.hadoop.mapred.ShuffleHandler", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.log-dirs", "/hadoop/yarn/log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.local-dirs", "/hadoop/yarn/local", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.container-monitor.interval-ms", "3000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.health-checker.interval-ms", "135000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.health-checker.script.timeout-ms", "60000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.log.retain-seconds", "604800", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.log-aggregation-enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.remote-app-log-dir", "/app-logs", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.remote-app-log-dir-suffix", "logs", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.log-aggregation.compression-type", "gz", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.delete.debug-delay-sec", "0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.log-aggregation.retain-seconds", "2592000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.admin-env", "MALLOC_ARENA_MAX=$MALLOC_ARENA_MAX", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.disk-health-checker.min-healthy-disks", "0.25", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.am.max-attempts", "2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.webapp.address", "localhost:8088", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.webapp.https.address", "localhost:8090", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.vmem-check-enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.log.server.url", "http:localhost:19888/jobhistory/logs", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.nodes.exclude-path", "/etc/hadoop/conf/yarn.exclude", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "manage.include.files", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.http.policy", "HTTP_ONLY", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.generic-application-history.store-class", "org.apache.hadoop.yarn.server.applicationhistoryservice.NullApplicationHistoryStore", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.leveldb-timeline-store.path", "/var/log/hadoop-yarn/timeline", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.webapp.address", "localhost:8188", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.webapp.https.address", "localhost:8190", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.address", "localhost:10200", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.ttl-enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.ttl-ms", "2678400000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.leveldb-timeline-store.ttl-interval-ms", "300000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.registry.zk.quorum", "localhost:2181", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.registry.dns.bind-port", "53", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.registry.dns.zone-mask", "255.255.255.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.registry.dns.zone-subnet", "172.17.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.registry.dns.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.registry.dns.domain-name", "EXAMPLE.COM", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.recovery.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.recovery.dir", "{{yarn_log_dir_prefix}}/nodemanager/recovery-state", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.client.nodemanager-connect.retry-interval-ms", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.client.nodemanager-connect.max-wait-ms", "60000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.recovery.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.work-preserving-recovery.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.store.class", "org.apache.hadoop.yarn.server.resourcemanager.recovery.ZKRMStateStore", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.zk-address", "localhost:2181", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.zk-state-store.parent-path", "/rmstore", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.zk-acl", "world:anyone:rwcda", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.work-preserving-recovery.scheduling-wait-ms", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.connect.retry-interval.ms", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.connect.max-wait.ms", "900000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.zk-retry-interval-ms", "1000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.zk-num-retries", "1000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.zk-timeout-ms", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.state-store.max-completed-applications", "${yarn.resourcemanager.max-completed-applications}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.fs.state-store.retry-policy-spec", "2000, 500", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.fs.state-store.uri", " ", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.ha.enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.linux-container-executor.resources-handler.class", "org.apache.hadoop.yarn.server.nodemanager.util.DefaultLCEResourcesHandler", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.linux-container-executor.cgroups.hierarchy", "/yarn", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.linux-container-executor.cgroups.mount", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.linux-container-executor.cgroups.mount-path", "/cgroup", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.linux-container-executor.cgroups.strict-resource-usage", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.resource.cpu-vcores", "8", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.resource.percentage-physical-cpu-limit", "80", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.node-labels.fs-store.retry-policy-spec", "2000, 500", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.disk-health-checker.min-free-space-per-disk-mb", "1000", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.disk-health-checker.max-disk-utilization-per-disk-percentage", "90", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.resource-plugins", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.resource-plugins.gpu.allowed-gpu-devices", "auto", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.resource-plugins.gpu.path-to-discovery-executables", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.log-aggregation.roll-monitoring-interval-seconds", "3600", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.log-aggregation.debug-enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.log-aggregation.num-log-files-per-app", "30", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.system-metrics-publisher.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.system-metrics-publisher.dispatcher.pool-size", "10", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.client.max-retries", "30", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.client.retry-interval-ms", "1000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.state-store-class", "org.apache.hadoop.yarn.server.timeline.recovery.LeveldbTimelineStateStore", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.leveldb-state-store.path", "/hadoop/yarn/timeline", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.leveldb-timeline-store.path", "/hadoop/yarn/timeline", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.leveldb-timeline-store.read-cache-size", "104857600", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.leveldb-timeline-store.start-time-read-cache-size", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.leveldb-timeline-store.start-time-write-cache-size", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.http-authentication.type", "simple", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.http-authentication.simple.anonymous.allowed", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.webapp.delegation-token-auth-filter.enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.bind-host", "0.0.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.bind-host", "0.0.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.bind-host", "0.0.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.node-labels.fs-store.root-dir", "/system/yarn/node-labels", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.minimum-allocation-vcores", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.maximum-allocation-vcores", "8", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.node-labels.enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.scheduler.monitor.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.recovery.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.version", "2.0f", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.versions", "1.5f,2.0f", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.system-metricspublisher.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.rm.system-metricspublisher.emit-container-events", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.recovery.supervised", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.store-class", "org.apache.hadoop.yarn.server.timeline.EntityGroupFSTimelineStore", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.entity-group-fs-store.active-dir", "/ats/active/", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.entity-group-fs-store.done-dir", "/ats/done/", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.entity-group-fs-store.group-id-plugin-classes", "org.apache.hadoop.yarn.applications.distributedshell.DistributedShellTimelinePlugin", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.entity-group-fs-store.summary-store", "org.apache.hadoop.yarn.server.timeline.RollingLevelDBTimelineStore", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.entity-group-fs-store.scan-interval-seconds", "60", conn);
endsendcommand(conn);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.log.server.web-service.url", "http:localhost:8188/ws/v1/applicationhistory", conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.entity-group-fs-store.cleaner-interval-seconds", "3600", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.entity-group-fs-store.retain-seconds", "604800", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.container-metrics.unregister-delay-ms", "60000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.entity-group-fs-store.group-id-plugin-classpath", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.monitor.capacity.preemption.total_preemption_per_round", "0.1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.monitor.capacity.preemption.natural_termination_factor", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.monitor.capacity.preemption.monitoring_interval", "15000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.linux-container-executor.nonsecure-mode.limit-users", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.runtime.linux.allowed-runtimes", "default,docker", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.runtime.linux.docker.allowed-container-networks", "host,none,bridge", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.runtime.linux.docker.default-container-network", "host", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.runtime.linux.docker.privileged-containers.allowed", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.runtime.linux.docker.privileged-containers.acl", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.runtime.linux.docker.capabilities", "CHOWN,DAC_OVERRIDE,FSETID,FOWNER,MKNOD,NET_RAW,SETGID,SETUID,SETFCAP,SETPCAP,NET_BIND_SERVICE,SYS_CHROOT,KILL,AUDIT_WRITE", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.webapp.ui2.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.http-cross-origin.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.webapp.cross-origin.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.webapp.cross-origin.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.resource-plugins.gpu.docker-plugin", "nvidia-docker-v1", conn);
endsendcommand(conn);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.resource-plugins.gpu.docker-plugin.nvidiadocker-v1.endpoint", "http:localhost:3476/v1.0/docker/cli", conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.webapp.api-service.enable", "true", conn);
endsendcommand(conn);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.service.framework.path", "/bigtop/apps/3.2.0/yarn/service-dep.tar.gz", conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.aux-services.timeline_collector.class", "org.apache.hadoop.yarn.server.timelineservice.collector.PerNodeTimelineCollectorsAuxService", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.reader.webapp.address", "localhost:8198", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.reader.webapp.https.address", "localhost:8199", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.hbase-schema.prefix", "prod.", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.hbase.configuration.file", "file:{{yarn_hbase_conf_dir}}/hbase-site.xml", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.hbase.coprocessor.jar.hdfs.location", "{{yarn_timeline_jar_location}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.monitor.capacity.preemption.intra-queue-preemption.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.ordering-policy.priority-utilization.underutilized-preemption.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resourcemanager.display.per-user-apps", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.service.system-service.dir", "/services", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.timeline-service.generic-application-history.save-non-am-container-meta-info", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.registry.dns.bind-address", "0.0.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.http.cross-origin.allowed-origins", "{{cross_origins}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.nodemanager.resourcemanager.connect.wait.secs", "1800", conn);
endsendcommand(conn);

 //Set yarn-log4j configuration parameters
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn_rm_summary_log_max_backup_size", "256", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn_rm_summary_log_number_of_backup_files", "20", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.log.dir", ".", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.mapreduce.jobsummary.logger", "${hadoop.root.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.mapreduce.jobsummary.log.file", "hadoop-mapreduce.jobsummary.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.server.resourcemanager.appsummary.log.file", "hadoop-mapreduce.jobsummary.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.server.resourcemanager.appsummary.logger", "${hadoop.root.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RMSUMMARY.File", "${yarn.log.dir}/${yarn.server.resourcemanager.appsummary.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RMSUMMARY.MaxFileSize", "{{yarn_rm_summary_log_max_backup_size}}MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RMSUMMARY.MaxBackupIndex", "{{yarn_rm_summary_log_number_of_backup_files}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RMSUMMARY.layout.ConversionPattern", "%d{ISO8601} %p %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.JSA.layout.ConversionPattern", "%d{yy/MM/dd HH:mm:ss} %p %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.logger.org.apache.hadoop.yarn.server.resourcemanager.RMAppManager$ApplicationSummary", "${yarn.server.resourcemanager.appsummary.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.additivity.org.apache.hadoop.yarn.server.resourcemanager.RMAppManager$ApplicationSummary", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "rm.audit.logger", "${hadoop.root.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.logger.org.apache.hadoop.yarn.server.resourcemanager.RMAuditLogger", "${rm.audit.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.additivity.org.apache.hadoop.yarn.server.resourcemanager.RMAuditLogger", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RMAUDIT.File", "${yarn.log.dir}/rm-audit.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.RMAUDIT.layout.ConversionPattern", "%d{ISO8601} %p %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "nm.audit.logger", "${hadoop.root.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.logger.org.apache.hadoop.yarn.server.nodemanager.NMAuditLogger", "${nm.audit.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.additivity.org.apache.hadoop.yarn.server.nodemanager.NMAuditLogger", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.NMAUDIT.File", "${yarn.log.dir}/nm-audit.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "log4j.appender.NMAUDIT.layout.ConversionPattern", "%d{ISO8601} %p %c{2}: %m%n", conn);
endsendcommand(conn);

 //Update Capacity Scheduler configuration parameters
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.maximum-applications", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.maximum-am-resource-percent", "0.2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.root.queues", "default", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.root.capacity", "100", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.root.default.capacity", "100", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.root.default.user-limit-factor", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.root.default.maximum-capacity", "100", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.root.default.state", "RUNNING", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.root.default.acl_submit_applications", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.root.default.acl_administer_jobs", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.root.acl_administer_queue", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.node-locality-del极ay", "40", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.scheduler.capacity.default.minimum-user-limit-percent", "100", conn);
endsendcommand(conn);

 //Set container-executor configuration parameters
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "docker_module_enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "docker_binary", "/usr/bin/docker", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "docker_allowed_devices", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "docker_allowed_ro-mounts", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "docker_allowed_rw-mounts", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "docker_allowed_volume-drivers", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "docker_privileged-containers_enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "docker_trusted_registries", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "min_user_id", "1000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "gpu_module_enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "cgroup_root", "/sys/fs/cgroup", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn_hierarchy", "yarn", conn);
endsendcommand(conn);

 //Set template reference (special handling for content property)
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "content.property-file-name", "container-executor.cfg.j2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "content.property-file-type", "text", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.db", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.aud极it.destination.db.jdbc.url", "{{audit_jdbc_url}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.db.user", "{{xa_audit_db_user}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.db.password", "crypted", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.db.jdbc.driver", "{{jdbc_driver}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.credential.provider.file", "jceks:file{{credential_file}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.db.batch.filespool.dir", "/var/log/hadoop/yarn/audit/db/spool", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.hdfs", "true", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.hdfs.batch.filespool.dir", "/var/log/hadoop/yarn/audit/hdfs/spool", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.solr", "false", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.destination.solr.batch.filespool.dir", "/var/log/hadoop/yarn/audit/solr/spool", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "xasecure.audit.provider.summary.enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.yarn.ambari.cluster.name", "{{cluster_name}}", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "hadoop.rpc.protection", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "common.name.for.certificate", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger-yarn-plugin-enabled", "No", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "REPOSITORY_CONFIG_USERNAME", "yarn", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "REPOSITORY_CONFIG_PASSWORD", "yarn", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "external_admin_username", "", conn);
endsendcommand(conn);
...
endsendcommand(conn);
 ...
 //Update Ranger YARN Security configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.yarn.service.name", "{{repo_name}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.yarn.policy.source.impl", "org.apache.ranger.admin.client.RangerAdminRESTClient", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.yarn.policy.rest.url", "{{policymgr_mgr_url}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.yarn.policy.rest.ssl.config.file", "/etc/hadoop/conf/ranger-policymgr-ssl-yarn.xml", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.yarn.policy.pollIntervalMs", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.plugin.yarn.policy.cache.dir", "/etc/ranger/{{repo_name}}/policycache", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "ranger.add-yarn-authorization", "true", conn);
endsendcommand(conn);

 //Update YARN resource types configuration
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resource-types", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HDFS, CONFIGURE, NULL, "yarn.resource-types.yarn.io_gpu.maximum-allocation", "8", conn);
endsendcommand(conn);
stop_stdout_capture();
 ...
 //Update simple properties in hbase-site.xml
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase_log_maxfilesize", "256", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase_log_maxbackupindex", "20", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase_security_log_maxfilesize", "256", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase_security_log_maxbackupindex", "20", conn);
endsendcommand(conn);

 //Process log4j.properties content line-by-line
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.root.logger", "INFO,console", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.security.logger", "INFO,console", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.log.dir", ".", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.log.file", "hbase.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.rootLogger", "${hbase.root.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.threshold", "ALL", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.DRFA", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.DRFA.File", "${hbase.log.dir}/${hbase.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.DRFA.DatePattern", ".yyyy-MM-dd", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.DRFA.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.DRFA.layout.ConversionPattern", "%d{ISO8601} %-5p [%t] %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.log.maxfilesize", "256MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.log.maxbackupindex", "20", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFA", "org.apache.log4j.RollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFA.File", "${hbase.log.dir}/${hbase.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFA.MaxFileSize", "${hbase.log.maxfilesize}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFA.MaxBackupIndex", "${hbase.log.maxbackupindex}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFA.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFA.layout.ConversionPattern", "%d{ISO8601} %-5p [%t] %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.security.log.file", "SecurityAuth.audit", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.security.log.maxfilesize", "256MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.security.log.maxbackupindex", "20", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFAS", "org.apache.log4j.RollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFAS.File", "${hbase.log.dir}/${hbase.security.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFAS.MaxFileSize", "${hbase.security.log.maxfilesize}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFAS.MaxBackupIndex", "${hbase.security.log.maxbackupindex}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFAS.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.RFAS.layout.ConversionPattern", "%d{ISO8601} %p %c: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.category.SecurityLogger", "${hbase.security.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.additivity.SecurityLogger", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.NullAppender", "org.apache.log4j.varia.NullAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.console", "org.apache.log4j.ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.console.target", "System.err", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.console.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.appender.console.layout.ConversionPattern", "%d{ISO8601} %-5p [%t] %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.logger.org.apache.zookeeper", "ERROR", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.logger.org.apache.hadoop.hbase", "ERROR", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.logger.org.apache.hadoop.hbase.zookeeper.ZKUtil", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "log4j.logger.org.apache.hadoop.hbase.zookeeper.ZooKeeperWatcher", "INFO", conn);
endsendcommand(conn);
 //Update HBase policy configuration parameters (to hbase-policy.xml)
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "security.client.protocol.acl", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "security.admin.protocol.acl", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "security.masterregion.protocol.acl", "*", conn);
endsendcommand(conn);
 //Core HBase Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.rootdir", "/apps/hbase/data", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.cluster.distributed", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.master.port", "16000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.tmp.dir", "/tmp/hbase-${user.name}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.local.dir", "${hbase.tmp.dir}/local", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.master.info.bindAddress", "0.0.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.master.info.port", "16010", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.regionserver.info.port", "16030", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.regionserver.handler.count", "30", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "phoenix.rpc.index.handler.count", "10", conn);
endsendcommand(conn);

 //Compaction and Memory Settings
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.hregion.majorcompaction", "604800000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.hregion.majorcompaction.jitter", "0.50", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.hregion.memstore.block.multiplier", "4", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.hregion.memstore.flush.size", "134217728", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.hregion.memstore.mslab.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.hregion.max.filesize", "10737418240", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.regionserver.global.memstore.size", "0.4", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hfile.block.cache.size", "0.40", conn);
endsendcommand(conn);

 //Client and Scanner Settings
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.client.scanner.caching", "100", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.client.retries.number", "35", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.rpc.timeout", "90000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.client.keyvalue.maxsize", "1048576", conn);
endsendcommand(conn);

 //ZooKeeper Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "zookeeper.session.timeout", "90000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.zookeeper.property.clientPort", "2181", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.zookeeper.quorum", "localhost", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.zookeeper.useMulti", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "zookeeper.znode.parent", "/hbase-unsecure", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "zookeeper.recovery.retry", "6", conn);
endsendcommand(conn);

 //Security Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.superuser", "hbase", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.security.authentication", "simple", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.security.authorization", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.rpc.protection", "authentication", conn);
endsendcommand(conn);

 //Coprocessor Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.coprocessor.region.classes", "org.apache.hadoop.hbase.security.access.SecureBulkLoadEndpoint", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.coprocessor.master.classes", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.coprocessor.regionserver.classes", "", conn);
endsendcommand(conn);

// RegionServer Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.regionserver.port", "16020", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.regionserver.executor.openregion.threads", "20", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.regionserver.wal.codec", "org.apache.hadoop.hbase.regionserver.wal.WALCellCodec", conn);
endsendcommand(conn);

 //Master Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.master.namespace.init.timeout", "2400000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.master.wait.on.regionservers.timeout", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.master.ui.readonly", "false", conn);
endsendcommand(conn);

// Store and Compaction Settings
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.hstore.compactionThreshold", "3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.hstore.compaction.max", "10", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.hstore.blockingStoreFiles", "100", conn);
endsendcommand(conn);

// Phoenix Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "phoenix.query.timeoutMs", "60000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "phoenix.functions.allowUserDefinedFunctions", " ", conn);
endsendcommand(conn);

 //Bucket Cache Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.bucketcache.ioengine", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.bucketcache.size", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.bucketcache.percentage.in.combinedcache", "", conn);
endsendcommand(conn);

 //RPC Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.region.server.rpc.scheduler.factory.class", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.rpc.controllerfactory.class", "", conn);
endsendcommand(conn);

 //Miscellaneous
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.defaults.for.version.skip", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "dfs.domain.socket.path", "/var/lib/hadoop-hdfs/dn_socket", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "hbase.bulkload.staging.dir", "/apps/hbase/staging", conn);
endsendcommand(conn);

 //Ranger HBase Audit Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.audit.is.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.audit.destination.hdfs", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.audit.destination.hdfs.dir", "hdfs:NAMENODE_HOSTNAME:8020/ranger/audit", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.audit.destination.hdfs.batch.filespool.dir", "/var/log/hbase/audit/hdfs/spool", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.audit.destination.solr", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.audit.destination.solr.urls", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.audit.destination.solr.zookeepers", "NONE", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.audit.destination.solr.batch.filespool.dir", "/var/log/hbase/audit/solr/spool", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.audit.provider.summary.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "ranger.plugin.hbase.ambari.cluster.name", "{{cluster_name}}", conn);
endsendcommand(conn);
// Ranger HBase Plugin Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "common.name.for.certificate", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "policy_user", "ambari-qa", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "ranger-hbase-plugin-enabled", "No", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "REPOSITORY_CONFIG_USERNAME", "hbase", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "REPOSITORY_CONFIG_PASSWORD", "hbase", conn);
endsendcommand(conn);

 //External Ranger Admin Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "external_admin_username", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "external_admin_password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "external_ranger_admin_username", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "external_ranger_admin_password", "", conn);
endsendcommand(conn);
 //Ranger Policy Manager SSL Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.policymgr.clientssl.keystore", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.policymgr.clientssl.keystore.password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.policymgr.clientssl.truststore", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.policymgr.clientssl.truststore.password", "changeit", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.policymgr.clientssl.keystore.credential.file", "jceks:file{{credential_file}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.policymgr.clientssl.truststore.credential.file", "jceks:file{{credential_file}}", conn);
endsendcommand(conn);
 //Ranger HBase Security Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "ranger.plugin.hbase.service.name", "{{repo_name}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "ranger.plugin.hbase.policy.source.impl", "org.apache.ranger.admin.client.RangerAdminRESTClient", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "ranger.plugin.hbase.policy.rest.url", "{{policymgr_mgr_url}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "ranger.plugin.hbase.policy.rest.ssl.config.file", "/etc/hbase/conf/ranger-policymgr-ssl.xml", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "ranger.plugin.hbase.policy.pollIntervalMs", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "ranger.plugin.hbase.policy.cache.dir", "/etc/ranger/{{repo_name}}/policycache", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HBASE, CONFIGURE, NULL, "xasecure.hbase.update.xapolicies.on.grant.revoke", "true", conn);
endsendcommand(conn);
stop_stdout_capture();
				break;
...
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "status", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "name", "BeelineLog4j2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "packages", "org.apache.hadoop.hive.ql.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.log.level", "{{hive_log_level}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.root.logger", "console", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appenders", "console", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.console.type", "Console", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.console.name", "console", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.console.target", "SYSTEM_ERR", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.console.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.console.layout.pattern", "%d{yy/MM/dd HH:mm:ss} [%t]: %p %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "loggers", "HiveConnection", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.HiveConnection.name", "org.apache.hive.jdbc.HiveConnection", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.HiveConnection.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "rootLogger.level", "${sys:hive.log.level}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "rootLogger.appenderRefs", "root", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "rootLogger.appenderRef.root.ref", "${sys:hive.root.logger}", conn);
endsendcommand(conn);
 //Atlas Hive Hook Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "atlas.hook.hive.synchronous", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "atlas.hook.hive.numRetries", "3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "atlas.hook.hive.minThreads", "5", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "atlas.hook.hive.maxThreads", "5", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "atlas.hook.hive.keepAliveTime", "10", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "atlas.hook.hive.queueSize", "1000", conn);
endsendcommand(conn);
 //Root configuration
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "status", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "name", "HiveExecLog4j2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "packages", "org.apache.hadoop.hive.ql.log", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.root.logger", "FA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.query.id", "hadoop", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.log.dir", "${sys:java.io.tmpdir}/${sys:user.name}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.log.file", "${sys:hive.query.id}.log", conn);
endsendcommand(conn);

 //Appender declarations
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appenders", "console, FA", conn);
endsendcommand(conn);
...
// File appender configuration
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.FA.type", "File", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.FA.name", "FA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.FA.fileName", "${sys:hive.log.dir}/${sys:hive.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.FA.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.FA.layout.pattern", "%d{ISO8601} %-5p [%t]: %c{2} (%F:%M(%L)) - %m%n", conn);
endsendcommand(conn);

 //Logger declarations
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "loggers", "NIOServerCnxn, ClientCnxnSocketNIO, DataNucleus, Datastore, JPOX", conn);
endsendcommand(conn);

// Individual logger configurations
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.NIOServerCnxn.name", "org.apache.zookeeper.server.NIOServerCnxn", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.NIOServerCnxn.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.ClientCnxnSocketNIO.name", "org.apache.zookeeper.ClientCnxnSocketNIO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.ClientCnxnSocketNIO.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.DataNucleus.name", "DataNucleus", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.DataNucleus.level", "ERROR", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.Datastore.name", "Datastore", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.Datastore.level", "ERROR", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.JPOX.name", "JPOX", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.JPOX.level", "ERROR", conn);
endsendcommand(conn);
...
endsendcommand(conn);
 //Standalone properties (outside content)
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive2_log_maxfilesize", "256", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive2_log_maxbackupindex", "30", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "name", "HiveLog4j2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "packages", "org.apache.hadoop.hive.ql.log", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.root.logger", "DRFA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.log.dir", "${sys:java.io.tmpdir}/${sys:user.name}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.log.file", "hive.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appenders", "console, DRFA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.console.type", "Console", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.type", "RollingFile", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.name", "DRFA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.fileName", "${sys:hive.log.dir}/${sys:hive.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.filePattern", "${sys:hive.log.dir}/${sys:hive.log.file}.%d{yyyy-MM-dd}_%i.gz", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.layout.pattern", "%d{ISO8601} %-5p [%t]: %c{2} (%F:%M(%L)) - %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.policies.type", "Policies", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.policies.time.type", "TimeBasedTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.policies.time.interval", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.policies.time.modulate", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.strategy.type", "DefaultRolloverStrategy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.strategy.max", "{{hive2_log_maxbackupindex}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.policies.fsize.type", "SizeBasedTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.policies.fsize.size", "{{hive2_log_maxfilesize}}MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "loggers", "NIOServerCnxn, ClientCnxnSocketNIO, DataNucleus, Datastore, JPOX", conn);
endsendcommand(conn);
...
endsendcommand(conn);
 //Tez Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "tez.session.am.dag.submit.timeout.secs", "0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.materializedviews.registry.impl", "DUMMY", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.max.start.attempts", "5", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.transport.mode", "binary", conn);
endsendcommand(conn);

 //File Format Settings
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.default.fileformat", "TextFile", conn);
endsendcommand(conn);

 //Metastore Security
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.sasl.enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.execute.setugi", "true", conn);
endsendcommand(conn);

 //Join Optimization
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.optimize.bucketmapjoin.sortedmerge", "false", conn);
endsendcommand(conn);

 //Tez Execution
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.container.size", "682", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.input.format", "org.apache.hadoop.hive.ql.io.HiveInputFormat", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.java.opts", "-server -Djava.net.preferIPv4Stack=true -XX:NewRatio=8 -XX:+UseNUMA -XX:+UseG1GC -XX:+ResizeTLAB -XX:+PrintGCDetails -verbose:gc -XX:+PrintGCTimeStamps", conn);
endsendcommand(conn);

// Transactions
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.txn.timeout", "300", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.compactor.initiator.on", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.compactor.worker.threads", "5", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.create.as.insert.only", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "metastore.create.as.acid", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.compactor.delta.num.threshold", "10", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.compactor.abortedtxn.threshold", "1000", conn);
endsendcommand(conn);

 //DataNucleus Cache
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "datanucleus.cache.level2.type", "none", conn);
endsendcommand(conn);

 //ZooKeeper
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.zookeeper.quorum", "localhost:2181", conn);
endsendcommand(conn);

 //Metastore Connection
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.connect.retries", "24", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.failure.retries", "24", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.client.connect.retry.delay", "5s", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.client.socket.timeout", "1800s", conn);
endsendcommand(conn);

// MapJoin
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.mapjoin.bucket.cache.size", "10000", conn);
endsendcommand(conn);

 //Security Delegation
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.cluster.delegation.token.store.class", "org.apache.hadoop.hive.thrift.ZooKeeperTokenStore", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.cluster.delegation.token.store.zookeeper.connectString", "localhost:2181", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.support.dynamic.service.discovery", "true", conn);
endsendcommand(conn);

// Execution Directories
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.scratchdir", "/tmp/hive", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.submitviachild", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.submit.local.task.via.child", "true", conn);
endsendcommand(conn);

// Compression
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.compress.output", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.compress.intermediate", "false", conn);
endsendcommand(conn);

 //Reducers
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.reducers.bytes.per.reducer", "67108864", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.reducers.max", "1009", conn);
endsendcommand(conn);

 //Hooks
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.pre.hooks", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.post.hooks", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.failure.hooks", "", conn);
endsendcommand(conn);

 //Parallel Execution
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.parallel", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.parallel.thread.number", "8", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.mapred.reduce.tasks.speculative.execution", "false", conn);
endsendcommand(conn);

 //Dynamic Partitioning
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.dynamic.partition", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.dynamic.partition.mode", "nonstrict", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.max.dynamic.partitions", "5000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.max.dynamic.partitions.pernode", "2000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.max.created.files", "100000", conn);
endsendcommand(conn);

 //Warehouse Directories
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.warehouse.dir", "/warehouse/tablespace/managed/hive", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.warehouse.external.dir", "/warehouse/tablespace/external/hive", conn);
endsendcommand(conn);

 //Lock Management
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.lock.manager", "", conn);
endsendcommand(conn);

 //Metastore URIs
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.uris", "thrift:localhost:9083", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.db.type", "{{hive_metastore_db_type}", conn);
endsendcommand(conn);

 //Database Connection (Sensitive)
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "javax.jdo.option.ConnectionPassword", "", conn); //  PASSWORD TYPE
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "javax.jdo.option.ConnectionURL", "jdbc:mysql:localhost/hive?createDatabaseIfNotExist=true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "javax.jdo.option.ConnectionDriverName", "com.mysql.jdbc.Driver", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "javax.jdo.option.ConnectionUserName", "hive", conn);
endsendcommand(conn);

 //Metastore Server
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.server.max.threads", "100000", conn);
endsendcommand(conn);

 //Kerberos
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.kerberos.keytab.file", "/etc/security/keytabs/hive.service.keytab", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.kerberos.principal", "hive/_HOST@EXAMPLE.COM", conn);
endsendcommand(conn);

 //Delegation Token Store
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.cluster.delegation.token.store.zookeeper.znode", "/hive/cluster/delegation", conn);
endsendcommand(conn);

 //Metastore Caching
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.cache.pinobjtypes", "Table,Database,Type,FieldSchema,Order", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.pre_event.listeners", "org.apache.hadoop.hive.ql.security.authorization.AuthorizationPreEventListener", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.authorization.storage.checks", "false", conn);
endsendcommand(conn);

 //Session Timeouts
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.idle.session.timeout", "1d", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.id极.operation.timeout", "6h", conn);
endsendcommand(conn);

 //Optimization Flags
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.limit.optimize.enable", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.strict.managed.tables", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.txn.strict.locking.mode", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.materializedview.rewriting.incremental", "false", conn);
endsendcommand(conn);

 //Map Aggregation
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.map.aggr", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.cbo.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.mapjoin.optimized.hashtable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.smbjoin.cache.rows", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.map.aggr.hash.percentmemory", "0.5", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.map.aggr.hash.force.flush.memory.threshold", "0.9", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.map.aggr.hash.min.reduction", "0.5", conn);
endsendcommand(conn);

 //Merge Operations
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.merge.mapfiles", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.merge.mapredfiles", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.merge.tezfiles", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.merge.size.per.task", "256000000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.merge.smallfiles.avgsize", "16000000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.merge.rcfile.block.level", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.merge.orcfile.stripe.level", "true", conn);
endsendcommand(conn);

 //ORC Format
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.orc.splits.include.file.footer", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.orc.compute.splits.num.threads", "10", conn);
endsendcommand(conn);

 //Auto Join Conversion
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.auto.convert.join", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.auto.convert.join.noconditionaltask", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.auto.convert.join.noconditionaltask.size", "52428800", conn);
endsendcommand(conn);

 //Limit Optimization
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.limit.optimize.enable", "true", conn);
endsendcommand(conn);

 //Tez CPU/Logging
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.cpu.vcores", "-1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.log.level", "INFO", conn);
endsendcommand(conn);

 //Join Enforcement
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.enforce.sortmergebucketmapjoin", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.auto.convert.sortmerge.join", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.auto.convert.sortmerge.join.to.mapjoin", "true", conn);
endsendcommand(conn);

// Query Optimization
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.optimize.constant.propagation", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.optimize.metadataonly", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.optimize.null.scan", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.optimize.bucketmapjoin", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.optimize.reducededuplication", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.optimize.reducededuplication.min.reducer", "4", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.optimize.sort.dynamic.partition", "false", conn);
endsendcommand(conn);

 //Statistics
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.stats.autogather", "极rue", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.stats.dbclass", "fs", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.stats.fetch.partition.stats", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.stats.fetch.column.stats", "false", conn);
endsendcommand(conn);

 //ZooKeeper Config
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.zookeeper.client.port", "2181", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.zookeeper.namespace", "hive_zookeeper_namespace", conn);
endsendcommand(conn);

 //Transactions
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.txn.manager", "org.apache.hadoop.hive.ql.lockmgr.DbTxnManager", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.txn.max.open.batch", "1000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.support.concurrency", "true", conn);
endsendcommand(conn);

 //CLI
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.cli.print.header", "false", conn);
endsendcommand(conn);

 //Compaction
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.compactor.worker.timeout", "86400", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.compactor.check.interval", "300", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.compactor.delta.pct.threshold", "0.1f", conn);
endsendcommand(conn);

// Fetch Task
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.fetch.task.conversion", "more", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.fetch.task.conversion.threshold", "1073741824", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.fetch.task.aggr", "false", conn);
endsendcommand(conn);

// Security
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.security.metastore.authorization.manager", "org.apache.hadoop.hive.ql.security.authorization.StorageBasedAuthorizationProvider", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.security.metastore.authorization.auth.reads", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.security.metastore.authenticator.manager", "org.apache.hadoop.hive.ql.security.HadoopDefaultMetastoreAuthenticator", conn);
endsendcommand(conn);

 //HiveServer2
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.logging.operation.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.logging.operation.log.location", "/tmp/hive/operation_logs", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.zookeeper.namespace", "hiveserver2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.thrift.http.port", "10001", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.thrift.port", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.thrift.sasl.qop", "auth", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.thrift.max.worker.threads", "500", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.allow.user.substitution", "true", conn);
endsendcommand(conn);

 //Authentication
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.authentication.spnego.keytab", "/etc/security/keytabs/spnego.service.keytab", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.authentication", "NONE", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.authentication.spnego.principal", "HTTP/_HOST@EXAMPLE.COM", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.event.db.notification.api.auth", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.enable.doAs", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.table.type.mapping", "CLASSIC", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.use.SSL", "false", conn);
endsendcommand(conn);

 //User Directories
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.user.install.directory", "/user/", conn);
endsendcommand(conn);

 //Vectorization
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.vectorized.groupby.maxentries", "100000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.merge.nway.joins", "false", conn);
endsendcommand(conn);

 //Tez Prewarming
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.prewarm.enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.prewarm.numcontainers", "3", conn);
endsendcommand(conn);

 //Tez Optimization
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.convert.join.bucket.mapjoin.tez", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.auto.reducer.parallelism", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.max.partition.factor", "2.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.min.partition.factor", "0.25", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.dynamic.partition.pruning", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.dynamic.partition.pruning.max.event.size", "1048576", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.dynamic.partition.pruning.max.data.size", "104857600", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.smb.number.waves", "0.5", conn);
endsendcommand(conn);

 //Database Schema
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "ambari.hive.db.schema.name", "hive", conn);
endsendcommand(conn);

 //Performance
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.vectorized.execution.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.optimize.index.filter", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.vectorized.groupby.checkinterval", "4096", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.vectorized.groupby.flush.percent", "0.1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.compute.query.using.stats", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.limit.pushdown.memory.usage", "0.04", conn);
endsendcommand(conn);

 //Tez Sessions
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.tez.sessions.per.default.queue", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.driver.parallel.compilation", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.tez.initialize.default.sessions", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.tez.default.queues", "default", conn);
endsendcommand(conn);

// Web UI
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.webui.port", "10002", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.webui.use.ssl", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.webui.enable.cors", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.webui.cors.allowed.headers", "X-Requested-With,Content-Type,Accept,Origin,X-Requested-By,x-requested-by", conn);
endsendcommand(conn);

// ORC Strategy
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.exec.orc.split.strategy", "HYBRID", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.vectorized.execution.reduce.enabled", "true", conn);
endsendcommand(conn);

// Authentication Providers
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.authentication.ldap.url", " ", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.authentication.ldap.baseDN", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.authentication.kerberos.keytab", "/etc/security/keytabs/hive.service.keytab", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.authentication.kerberos.principal", "hive/_HOST@EXAMPLE.COM", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.authentication.pam.services", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.custom.authentication.class", "", conn);
endsendcommand(conn);

 //DataNucleus
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "datanucleus.autoCreateSchema", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "datanucleus.fixedDatastore", "true", conn);
endsendcommand(conn);

// Managed Table Format
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.default.fileformat.managed", "ORC", conn);
endsendcommand(conn);

 //Atlas Hooks
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "atlas.hook.hive.minThreads", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "atlas.hook.hive.maxThreads", "1", conn);
endsendcommand(conn);

 //Query Data Directory
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.hook.proto.base-directory", "{hive_metastore_warehouse_external_dir}/sys.db/query_data/", conn);
endsendcommand(conn);

 //Interactive Execution
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.execution.mode", "container", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.input.generate.consistent.splits", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.exec.print.summary", "true", conn);
endsendcommand(conn);

 //Vectorized MapJoin
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.vectorized.execution.mapjoin.native.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.vectorized.execution.mapjoin.minmax.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.vectorized.execution.mapjoin.native.fast.hashtable.enabled", "true", conn);
endsendcommand(conn);

 ///Dynamic Partition HashJoin
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.optimize.dynamic.partition.hashjoin", "true", conn);
endsendcommand(conn);

 //Event Listeners
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.event.listeners", "", conn);
endsendcommand(conn);

 //MapJoin Hybrid
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.mapjoin.hybridgrace.hashtable", "false", conn);
endsendcommand(conn);

// Tez Features
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.cartesian-product.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.tez.bucket.pruning", "true", conn);
endsendcommand(conn);

// Metrics
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.service.metrics.codahale.reporter.classes", "org.apache.hadoop.hive.common.metrics.metrics2.JsonFileMetricsReporter,org.apache.hadoop.hive.common.metrics.metrics2.JmxMetricsReporter,org.apache.hadoop.hive.common.metrics.metrics2.Metrics2Reporter", conn);
endsendcommand(conn);

 //Replication
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.dml.events", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.repl.cm.enabled", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.transactional.event.listeners", "org.apache.hive.hcatalog.listener.DbNotificationListener", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.repl.cmrootdir", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.repl.rootdir", "", conn);
endsendcommand(conn);

 //Vectorized UDF
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.vectorized.adaptor.usage.mode", "chosen", conn);
endsendcommand(conn);

 //Metastore Metrics
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.metastore.metrics.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.server2.metrics.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.service.metrics.reporter", "HADOOP2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.service.metrics.hadoop2.component", "hivemetastore", conn);
endsendcommand(conn);
...
endsendcommand(conn);
 //Security Configuration
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.security.authenticator.manager", "org.apache.hadoop.hive.ql.security.SessionStateUserAuthenticator", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.security.authorization.manager", "org.apache.hadoop.hive.ql.security.authorization.plugin.sqlstd.SQLStdHiveAuthorizerFactory", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.security.authorization.enabled", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.conf.restricted.list", "hive.security.authenticator.manager,hive.security.authorization.manager,hive.users.in.admin.role", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive.service.metrics.hadoop2.component", "hiveserver2", conn);
endsendcommand(conn);
 //Standalone log rotation settings
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "llap_cli_log_maxfilesize", "256", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "llap_cli_log_maxbackupindex", "30", conn);
endsendcommand(conn);

 //Log4j2 configuration content
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "status", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "name", "LlapCliLog4j2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "packages", "org.apache.hadoop.hive.ql.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.log.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.root.logger", "console", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.log.file", "llap-cli.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.hive.llapstatus.consolelogger.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appenders", "console, DRFA, llapstatusconsole", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.console.type", "Console", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.console.layout.pattern", "%p %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.llapstatusconsole.type", "Console", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.llapstatusconsole.name", "llapstatusconsole", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.llapstatusconsole.target", "SYSTEM_OUT", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.llapstatusconsole.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.llapstatusconsole.layout.pattern", "%m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.type", "RollingRandomAccessFile", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.name", "DRFA", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.filePattern", "${sys:hive.log.dir}/${sys:hive.log.file}.%d{yyyy-MM-dd}_%i", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.layout.pattern", "%d{ISO8601} %5p [%t] %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.policies.type", "Policies", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.strategy.max", "{{llap_cli_log_maxbackupindex}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.policies.fsize.type", "SizeBasedTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.DRFA.policies.fsize.size", "{{llap_cli_log_maxfilesize}}MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "loggers", "ZooKeeper, DataNucleus, Datastore, JPOX, HadoopConf, LlapStatusServiceDriverConsole", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.ZooKeeper.name", "org.apache.zookeeper", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.ZooKeeper.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.DataNucleus.name", "DataNucleus", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.HadoopConf.name", "org.apache.hadoop.conf.Configuration", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.HadoopConf.level", "ERROR", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapStatusServiceDriverConsole.name", "LlapStatusServiceDriverConsole", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapStatusServiceDriverConsole.additivity", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapStatusServiceDriverConsole.level", "${sys:hive.llapstatus.consolelogger.level}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "rootLogger.level", "${sys:hive.log.level}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "rootLogger.appenderRefs", "root, DRFA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "rootLogger.appenderRef.root.ref", "${sys:hive.root.logger}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "rootLogger.appenderRef.DRFA.ref", "DRFA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapStatusServiceDriverConsole.appenderRefs", "llapstatusconsole, DRFA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapStatusServiceDriverConsole.appenderRef.llapstatusconsole.ref", "llapstatusconsole", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapStatusServiceDriverConsole.appenderRef.DRFA.ref", "DRFA", conn);
endsendcommand(conn);

 //Standalone log rotation settings
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive_llap_log_maxfilesize", "256", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "hive_llap_log_maxbackupindex", "240", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "name", "LlapDaemonLog4j2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "packages", "org.apache.hadoop.hive.ql.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.llap.daemon.log.level", "{{hive_log_level}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.llap.daemon.root.logger", "console", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.llap.daemon.log.dir", ".", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.llap.daemon.log.file", "llapdaemon.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.llap.daemon.historylog.file", "llapdaemon_history.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.llap.daemon.log.maxfilesize", "{{hive_llap_log_maxfilesize}}MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "property.llap.daemon.log.maxbackupindex", "{{hive_llap_log_maxbackupindex}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appenders", "console, RFA, HISTORYAPPENDER, query-routing", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.console.type", "Console", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.console.layout.pattern", "%d{ISO8601} %5p [%t (%X{fragmentId})] %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.type", "RollingRandomAccessFile", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.name", "RFA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.fileName", "${sys:llap.daemon.log.dir}/${sys:llap.daemon.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.filePattern", "${sys:llap.daemon.log.dir}/${sys:llap.daemon.log.file}_%d{yyyy-MM-dd-HH}_%i.done", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.layout.pattern", "%d{ISO8601} %-5p [%t (%X{fragmentId})] %c: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.policies.type", "Policies", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.policies.time.type", "TimeBasedTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.policies.time.interval", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.policies.time.modulate", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.policies.size.type", "SizeBasedTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.policies.size.size", "${sys:llap.daemon.log.maxfilesize}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.strategy.type", "DefaultRolloverStrategy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.RFA.strategy.max", "${sys:llap.daemon.log.maxbackupindex}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.type", "RollingRandomAccessFile", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.name", "HISTORYAPPENDER", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.fileName", "${sys:llap.daemon.log.dir}/${sys:llap.daemon.historylog.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.filePattern", "${sys:llap.daemon.log.dir}/${sys:llap.daemon.historylog.file}_%d{yyyy-MM-dd}_%i.done", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.layout.pattern", "%m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.policies.type", "Policies", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.policies.size.type", "SizeBasedTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.policies.size.size", "${sys:llap.daemon.log.maxfilesize}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.policies.time.type", "TimeBasedTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.policies.time.interval", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.policies.time.modulate", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.strategy.type", "DefaultRolloverStrategy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.HISTORYAPPENDER.strategy.max", "${sys:llap.daemon.log.maxbackupindex}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.type", "Routing", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.name", "query-routing", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.type", "Routes", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.pattern", "$${ctx:queryId}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.purgePolicy.type", "LlapRoutingAppenderPurgePolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.purgePolicy.name", "llapLogPurgerQueryRouting", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-default.type", "Route", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-default.key", "$${ctx:queryId}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-default.ref", "RFA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-mdc.type", "Route", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-mdc.file-mdc.type", "LlapWrappedAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-mdc.file-mdc.name", "IrrelevantName-query-routing", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-mdc.file-mdc.app.type", "RandomAccessFile", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-mdc.file-mdc.app.name", "file-mdc", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-mdc.file-mdc.app.fileName", "${sys:llap.daemon.log.dir}/${ctx:queryId}-${ctx:dagId}.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-mdc.file-mdc.app.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "appender.query-routing.routes.route-mdc.file-mdc.app.layout.pattern", "%d{ISO8601} %5p [%t (%X{fragmentId})] %c{2}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "loggers", "PerfLogger, EncodedReader, NIOServerCnxn, ClientCnxnSocketNIO, DataNucleus, Datastore, JPOX, HistoryLogger, LlapIoImpl, LlapIoOrc, LlapIoCache, LlapIoLocking, TezSM, TezSS, TezHC", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.TezSM.name", "org.apache.tez.runtime.library.common.shuffle.impl.ShuffleManager.fetch", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.TezSM.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.TezSS.name", "org.apache.tez.runtime.library.common.shuffle.orderedgrouped.ShuffleScheduler.fetch", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.TezSS.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.TezHC.name", "org.apache.tez.http.HttpConnection.url", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.TezHC.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.PerfLogger.name", "org.apache.hadoop.hive.ql.log.PerfLogger", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.PerfLogger.level", "DEBUG", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.EncodedReader.name", "org.apache.hadoop.hive.ql.io.orc.encoded.EncodedReaderImpl", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.EncodedReader.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapIoImpl.name", "LlapIoImpl", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapIoImpl.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapIoOrc.name", "LlapIoOrc", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapIoOrc.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapIoCache.name", "LlapIoCache", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapIoCache.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapIoLocking.name", "LlapIoLocking", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.LlapIoLocking.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.NIOServerCnxn.name", "org.apache.zookeeper.server.NIOServerCnxn", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.HistoryLogger.name", "org.apache.hadoop.hive.llap.daemon.HistoryLogger", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.HistoryLogger.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.HistoryLogger.additivity", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.HistoryLogger.appenderRefs", "HistoryAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "logger.HistoryLogger.appenderRef.HistoryAppender.ref", "HISTORYAPPENDER", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "rootLogger.level", "${sys:llap.daemon.log.level}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "rootLogger.appenderRefs", "root", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(HIVE, CONFIGURE, NULL, "rootLogger.appenderRef.root.ref", "${sys:llap.daemon.root.logger}", conn);
endsendcommand(conn);
stop_stdout_capture();
				break;
 ...
 //Update Kafka configuration parameters in 'server.properties'
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log.dirs", "/kafka-logs", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "zookeeper.connect", "localhost:2181", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "message.max.bytes", "1000000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "num.network.threads", "3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "num.io.threads", "8", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "queued.max.requests", "500", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "socket.send.buffer.bytes", "102400", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "socket.receive.buffer.bytes", "102400", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "socket.request.max.bytes", "104857600", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "num.partitions", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log.segment.bytes", "1073741824", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log.roll.hours", "168", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log.retention.bytes", "-1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log.retention.hours", "168", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log.retention.check.interval.ms", "600000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log.index.size.max.bytes", "10485760", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log.index.interval.bytes", "4096", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "auto.create.topics.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "controller.socket.timeout.ms", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "controller.message.queue.size", "10", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "default.replication.factor", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "replica.lag.time.max.ms", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "replica.lag.max.messages", "4000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "replica.socket.timeout.ms", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "replica.socket.receive.buffer.bytes", "65536", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "replica.fetch.max.bytes", "1048576", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "replica.fetch.wait.max.ms", "500", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "replica.fetch.min.bytes", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "num.replica.fetchers", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "replica.high.watermark.checkpoint.interval.ms", "5000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "fetch.purgatory.purge.interval.requests", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "producer.purgatory.purge.interval.requests", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "zookeeper.session.timeout.ms", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "zookeeper.connection.timeout.ms", "25000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "zookeeper.sync.time.ms", "200极", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "controlled.shutdown.max.retries", "3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "controlled.shutdown.retry.backoff.ms", "5000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.metrics.reporters", "{{metrics_reporters}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.ganglia.metrics.reporter.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.ganglia.metrics.host", "localhost", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.ganglia.metrics.port", "8671", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.ganglia.metrics.group", "kafka", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.reporter.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.hosts", "{{ams_collector_hosts}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.port", "{{metric_collector_port}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.protocol", "{{metric_collector_protocol}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.truststore.path", "{{metric_truststore_path}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.truststore.type", "{{metric_truststore_type}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.truststore.password", "{{metric_truststore_password}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.reporter.sendInterval", "5900", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.maxRowCacheSize", "10000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.host_in_memory_aggregation", "{{host_in_memory_aggregation}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.host_in_memory_aggregation_port", "{{host_in_memory_aggregation_port}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.timeline.metrics.host_in_memory_aggregation_protocol", "{{host_in_memory_aggregation_protocol}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "listeners", "PLAINTEXT:localhost:9092", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "raw.listeners", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "controlled.shutdown.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "auto.leader.rebalance.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "num.recovery.threads.per.data.dir", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "min.insync.replicas", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "leader.imbalance.per.broker.percentage", "10", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "leader.imbalance.check.interval.seconds", "300", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "offset.metadata.max.bytes", "4096", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "offsets.load.buffer.size", "5242880", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "offsets.topic.replication.factor", "3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "offsets.topic.num.partitions", "50", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "offsets.topic.segment.bytes", "104857600", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "offsets.topic.compression.codec", "0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "offsets.retention.minutes", "86400000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "offsets.retention.check.interval.ms", "600000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "offsets.commit.timeout.ms", "5000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "offsets.commit.required.acks", "-1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "delete.topic.enable", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "compression.type", "producer", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "external.kafka.metrics.exclude.prefix", "kafka.network.RequestMetrics,kafka.server.DelayedOperationPurgatory,kafka.server.BrokerTopicMetrics.BytesRejectedPerSec", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "external.kafka.metrics.include.prefix", "kafka.network.RequestMetrics.ResponseQueueTimeMs.request.OffsetCommit.98percentile,kafka.network.RequestMetrics.ResponseQueueTimeMs.request.Offsets.95percentile,kafka.network.RequestMetrics.ResponseSendTimeMs.request.Fetch.95percentile,kafka.network.RequestMetrics.RequestsPerSec.request", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "sasl.enabled.mechanisms", "GSSAPI", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "security.inter.broker.protocol", "PLAINTEXT", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "sasl.mechanism.inter.broker.protocol", "GSSAPI", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ssl.client.auth", "none", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ssl.key.password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ssl.keystore.location", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ssl.keystore.password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ssl.truststore.location", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ssl.truststore.password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "producer.metrics.enable", "false", conn);
endsendcommand(conn);
 //Update Kafka Log4j configuration in 'log4j.properties'
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "kafka.logs.dir", "logs", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.rootLogger", "INFO, stdout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.stdout", "org.apache.log4j.ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.stdout.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.stdout.layout.ConversionPattern", "[%d{ISO8601}] %p %m (%c)%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.kafkaAppender", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.kafkaAppender.DatePattern", "'.'yyyy-MM-dd-HH", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.kafkaAppender.File", "${kafka.logs.dir}/server.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.kafkaAppender.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.kafkaAppender.layout.ConversionPattern", "[%d{ISO8601}] %p %m (%c)%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.kafkaAppender.MaxFileSize", "256MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.kafkaAppender.MaxBackupIndex", "20", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.stateChangeAppender", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.stateChangeAppender.DatePattern", "'.'yyyy-MM-dd-HH", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.stateChangeAppender.File", "${kafka.logs.dir}/state-change.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.stateChangeAppender.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.stateChangeAppender.layout.ConversionPattern", "[%d{ISO8601}] %p %m (%c)%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.requestAppender", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.requestAppender.DatePattern", "'.'yyyy-MM-dd-HH", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.requestAppender.File", "${kafka.logs.dir}/kafka-request.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.requestAppender.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.requestAppender.layout.ConversionPattern", "[%d{ISO8601}] %p %m (%c)%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.cleanerAppender", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.cleanerAppender.DatePattern", "'.'yyyy-MM-dd-HH", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4极j.appender.cleanerAppender.File", "${kafka.logs.dir}/log-cleaner.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.cleanerAppender.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.cleanerAppender.layout.ConversionPattern", "[%d{ISO8601}] %p %m (%c)%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.controllerAppender", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.controllerAppender.DatePattern", "'.'yyyy-MM-dd-HH", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.controllerAppender.File", "${kafka.logs.dir}/controller.log", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.controllerAppender.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.controllerAppender.layout.ConversionPattern", "[%d{ISO8601}] %p %m (%c)%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.controllerAppender.MaxFileSize", "256MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.appender.controllerAppender.MaxBackupIndex", "20", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.logger.kafka", "INFO, kafkaAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.logger.kafka.network.RequestChannel$", "WARN, requestAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.additivity.kafka.network.RequestChannel$", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.logger.kafka.request.logger", "WARN, requestAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.additivity.kafka.request.logger", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.logger.kafka.controller", "TRACE, controllerAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.additivity.kafka.controller", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.logger.kafka.log.LogCleaner", "INFO, cleanerAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.additivity.kafka.log.LogCleaner", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.logger.state.change.logger", "TRACE, stateChangeAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log4j.additivity.state.change.logger", "false", conn);
endsendcommand(conn);
 //Update Kafka Client JAAS configuration in 'kafka_client_jaas.conf'
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "content", 
  "  {% if kerberos_security_enabled %}\n"
  "  KafkaClient {\n"
  "  com.sun.security.auth.module.Krb5LoginModule required\n"
  "  useTicketCache=true\n"
...
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "content", 
 ...
 //Update Ranger-Kafka audit configuration
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.audit.is.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.audit.destination.hdfs", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.audit.destination.hdfs.dir", "hdfs:NAMENODE_HOSTNAME:8020/ranger/audit", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.audit.destination.hdfs.batch.filespool.dir", "/var/log/kafka/audit/hdfs/spool", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.audit.destination.solr", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.audit.destination.solr.urls", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.audit.destination.solr.zookeepers", "NONE", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.audit.destination.solr.batch.filespool.dir", "/var/log/kafka/audit/solr/spool", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.audit.provider.summary.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ranger.plugin.kafka.ambari.cluster.name", "{{cluster_name}}", conn);
endsendcommand(conn);
 //Update Ranger-Kafka plugin properties
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "policy_user", "ambari-qa", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "hadoop.rpc.protection", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "common.name.for.certificate", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "zookeeper.connect", "localhost:2181", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ranger-kafka-plugin-enabled", "No", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "REPOSITORY_CONFIG_PASSWORD", "kafka", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "external_admin_username", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "external_admin_password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "external_ranger_admin_username", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "external_ranger_admin_password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "REPOSITORY_CONFIG_USERNAME", "kafka", conn);
endsendcommand(conn);
// Update Ranger-Kafka Policy Manager SSL configuration
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.policymgr.clientssl.keystore", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.policymgr.clientssl.keystore.password", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.policymgr.clientssl.truststore", "", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.policymgr.clientssl.truststore.password", "changeit", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.policymgr.clientssl.keystore.credential.file", "jceks:file/{{credential_file}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "xasecure.policymgr.clientssl.truststore.credential.file", "jceks:file/{{credential_file}}", conn);
endsendcommand(conn);
 //Update Ranger-Kafka security configuration
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ranger.plugin.kafka.service.name", "{{repo_name}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ranger.plugin.kafka.policy.source.impl", "org.apache.ranger.admin.client.RangerAdminRESTClient", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ranger.plugin.kafka.policy.rest.url", "{{policymgr_mgr_url}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ranger.plugin.kafka.policy.rest.ssl.config.file", "/etc/kafka/conf/ranger-policymgr-ssl.xml", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ranger.plugin.kafka.policy.pollIntervalMs", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "ranger.plugin.kafka.policy.cache.dir", "/etc/ranger/{{repo_name}}/policycache", conn);
endsendcommand(conn);
 //Core Kafka Broker Configuration
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "log.dirs", "/kafka-logs", conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "zookeeper.sync.time.ms", "2000", conn);
endsendcommand(conn);
 //Shutdown Configuration
conn = startcommand(host, port);
SendComponentActionCommand(KAFKA, CONFIGURE, NULL, "controlled.shutdown.enable", "true", conn);
...
//		       Update Livy environment setting
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.environment", "production", conn);
endsendcommand(conn);

// Set Livy server port
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.server.port", "8999", conn);
endsendcommand(conn);

 //Configure session timeout
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.server.session.timeout", "3600000", conn);
endsendcommand(conn);

 //Enable user impersonation (first occurrence)
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.impersonation.enabled", "true", conn);
endsendcommand(conn);
...
endsendcommand(conn);
 ...
 //Activate CSRF protection
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.server.csrf_protection.enabled", "true", conn);
endsendcommand(conn);

 //Define Spark master
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.spark.master", "yarn", conn);
endsendcommand(conn);

 //Set Spark deploy mode
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.spark.deploy-mode", "cluster", conn);
endsendcommand(conn);

// Configure HiveContext in interpreter
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.repl.enableHiveContext", "true", conn);
endsendcommand(conn);

 //Set recovery mode
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.server.recovery.mode", "recovery", conn);
endsendcommand(conn);

 //Define recovery state storage type
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.server.recovery.state-store", "filesystem", conn);
endsendcommand(conn);

 //Configure recovery storage path
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.server.recovery.state-store.url", "/livy-recovery", conn);
endsendcommand(conn);

 //Enable access control
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.server.access-control.enabled", "true", conn);
endsendcommand(conn);

 //Set RSC launcher address (note: space character as value)
conn = startcommand(host, port);
SendComponentActionCommand(LIVY, CONFIGURE, NULL, "livy.rsc.launcher.address", " ", conn);
endsendcommand(conn);

 //Set complete log4j configuration content
conn = startcommand(host, port);
SendComponentActionCommand(
    LIVY, CONFIGURE, NULL, 
    "content", 
    "\n            # Set everything to be logged to the console\n            log4j.rootCategory=INFO, console\n            log4j.appender.console=org.apache.log4j.ConsoleAppender\n            log4j.appender.console.target=System.err\n            log4j.appender.console.layout=org.apache.log4j.PatternLayout\n            log4j.appender.console.layout.ConversionPattern=%d{yy/MM/dd HH:mm:ss} %p %c{1}: %m%n\n\n            log4j.logger.org.eclipse.jetty=WARN\n        ",
    conn
...
stop_stdout_capture();
 ...
 //Update Solr configuration parameters (from XML translation)
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "log_maxfilesize", "10", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "log_maxbackupindex", "9", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "content", "", conn);
endsendcommand(conn);

// Update Solr security configuration parameters (from XML translation)
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr_ranger_audit_service_users", "{default_ranger_audit_users}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr_role_ranger_admin", "ranger_admin_user", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr_role_ranger_audit", "ranger_audit_user", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr_role_atlas", "atlas_user", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr_role_logsearch", "logsearch_user", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr_role_logfeeder", "logfeeder_user", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr_role_dev", "dev", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr_security_manually_managed", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SOLR, CONFIGURE, NULL, "content", "", conn);
endsendcommand(conn);
...
stop_stdout_capture();
        ...
        exit(EXIT_FAILURE);
			...
			conn = startcommand(host, port);
				SendComponentActionCommand(PIG, CONFIGURE, NULL, "pig.default.mapred.partitioner", 
						"org.apache.pig.backend.hadoop.executionengine.mapReduceLayer.PigTotalOrderPartitioner", conn);
				SendComponentActionCommand(PIG, CONFIGURE, NULL, "pig.tmpfilecompression", "true", conn);
				SendComponentActionCommand(PIG, CONFIGURE, NULL, "pig.maxCombinedSplitSize", "134217728", conn);
				stop_stdout_capture();
				break;
        ...
        exit(EXIT_FAILURE);
			...
			conn = startcommand(host, port);
				SendComponentActionCommand(PRESTO, CONFIGURE, NULL, "http-server.http.port", "8080", conn);
				SendComponentActionCommand(PRESTO, CONFIGURE, NULL, "query.max-memory", "8GB", conn);
				SendComponentActionCommand(PRESTO, CONFIGURE, NULL, "discovery.uri", "http:localhost:8080", conn);
				stop_stdout_capture();
				break;
        ...
        exit(EXIT_FAILURE);
			...
			conn = startcommand(host, port);
				SendComponentActionCommand(ATLAS, CONFIGURE, NULL, "atlas.graph.storage.backend", "hbase", conn);
				SendComponentActionCommand(ATLAS, CONFIGURE, NULL, "atlas.graph.storage.hostname", "localhost:2181", conn);
				SendComponentActionCommand(ATLAS, CONFIGURE, NULL, "atlas.graph.index.search.backend", "solr", conn);
				SendComponentActionCommand(ATLAS, CONFIGURE, NULL, "atlas.graph.index.search.solr.mode", "cloud", conn);
				SendComponentActionCommand(ATLAS, CONFIGURE, NULL, "atlas.graph.index.search.solr.zookeeper-url", "localhost:2181/solr", conn);
				SendComponentActionCommand(ATLAS, CONFIGURE, NULL, "atlas.audit.hbase.zookeeper.quorum", "localhost", conn);
				stop_stdout_capture();
				break;
        ...
        exit(EXIT_FAILURE);
			...
			conn = startcommand(host, port);
				SendComponentActionCommand(RANGER, CONFIGURE, NULL, "ranger.jpa.jdbc.url", "jdbc:mysql:localhost/ranger", conn);
				SendComponentActionCommand(RANGER, CONFIGURE, NULL, "ranger.jpa.jdbc.user", "rangeradmin", conn);
				SendComponentActionCommand(RANGER, CONFIGURE, NULL, "ranger.jpa.jdbc.password", "rangeradmin", conn);
				stop_stdout_capture();
				break;
        ...
        exit(EXIT_FAILURE);
			...
			conn = startcommand(host, port);
				SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr.autoSoftCommit.maxTime", "1000", conn);
				SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr.autoCommit.maxTime", "15000", conn);
				SendComponentActionCommand(SOLR, CONFIGURE, NULL, "solr.ulog.numRecordsToKeep", "10000", conn);
				stop_stdout_capture();
				break;
 ...
 //Update Spark configuration parameters from XML
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.yarn.queue", "default", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.history.provider", "org.apache.spark.deploy.history.FsHistoryProvider", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.history.ui.port", "18081", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.history.fs.logDirectory", "hdfs:/spark-history/", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.history.kerberos.principal", "none", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.history.kerberos.keytab", "none", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.eventLog.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.eventLog.dir", "hdfs:/spark-history/", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.yarn.historyServer.address", "{{spark_history_server_host}}:{{spark_history_ui_port}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.scheduler.allocation.file", "file:/{{spark_conf_dir}}/spark-thrift-fairscheduler.xml", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.scheduler.mode", "FAIR", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.hadoop.cacheConf", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.yarn.executor.failuresValidityInterval", "2h", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.yarn.maxAppAttempts", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.history.fs.cleaner.enabled", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.history.fs.cleaner.interval", "7d", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.history.fs.cleaner.maxAge", "90d", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.sql.statistics.fallBackToHdfs", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.sql.autoBroadcastJoinThreshold", "10MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.io.compression.lz4.blockSize", "128kb", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.sql.orc.filterPushdown", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.sql.hive.convertMetastoreOrc", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.shuffle.io.backLog", "8192", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.shuffle.file.buffer", "1m", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.master", "yarn", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.executor.extraJavaOptions", "-XX:+UseNUMA", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.sql.warehouse.dir", "{{spark_warehouse_dir}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.sql.hive.metastore.version", "3.1.3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.sql.hive.metastore.jars", "{{hive_home}}/lib/*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "spark.history.store.path", "/var/lib/spark/shs_db", conn);
endsendcommand(conn);



conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "log4j.rootCategory", "INFO, console", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "log4j.appender.console", "org.apache.log4j.ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "log4j.appender.console.target", "System.err", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "log4j.appender.console.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "log4j.appender.console.layout.ConversionPattern", "%d{yy/MM/dd HH:mm:ss} %p %c{1}: %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "log4j.logger.org.eclipse.jetty", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "log4j.logger.org.eclipse.jetty.util.component.AbstractLifeCycle", "ERROR", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "log4j.logger.org.apache.spark.repl.SparkIMain$exprTyper", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "log4j.logger.org.apache.spark.repl.SparkILoop$SparkILoopInterpreter", "INFO", conn);
endsendcommand(conn);


 //Enable JmxSink for all instances
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "*.sink.jmx.class", "org.apache.spark.metrics.sink.JmxSink", conn);
endsendcommand(conn);

 //Enable ConsoleSink for all instances with polling configuration
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "*.sink.console.class", "org.apache.spark.metrics.sink.ConsoleSink", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "*.sink.console.period", "10", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "*.sink.console.unit", "seconds", conn);
endsendcommand(conn);

// Master-specific console sink configuration
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "master.sink.console.period", "15", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "master.sink.console.unit", "seconds", conn);
endsendcommand(conn);

// CSV Sink configuration
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "*.sink.csv.class", "org.apache.spark.metrics.sink.CsvSink", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "*.sink.csv.period", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "*.sink.csv.unit", "minutes", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "*.sink.csv.directory", "/tmp/", conn);
endsendcommand(conn);

// Worker-specific CSV configuration
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "worker.sink.csv.period", "10", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "worker.sink.csv.unit", "minutes", conn);
endsendcommand(conn);

 //JVM Source configuration for all components
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "master.source.jvm.class", "org.apache.spark.metrics.source.JvmSource", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "worker.source.jvm.class", "org.apache.spark.metrics.source.JvmSource", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "driver.source.jvm.class", "org.apache.spark.metrics.source.JvmSource", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "executor.source.jvm.class", "org.apache.spark.metrics.source.JvmSource", conn);
endsendcommand(conn);
...
const char* fair_scheduler_content = 
 ...
 //Use a special parameter name to indicate this is file content
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "fairscheduler.xml.content", fair_scheduler_content, conn);
...
// Hive Server 2 impersonation settings
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "hive.server2.enable.doAs", "false", conn);
endsendcommand(conn);

 //Metastore client settings
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "hive.metastore.client.socket.timeout", "1800", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "hive.metastore.client.connect.retry.delay", "5", conn);
endsendcommand(conn);

// Hive Server 2 connection settings
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "hive.server2.thrift.port", "10016", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "hive.server2.thrift.http.port", "10002", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "hive.server2.transport.mode", "binary", conn);
endsendcommand(conn);

// Metastore and execution settings
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "metastore.catalog.default", "hive", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "hive.load.data.owner", "spark", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(SPARK, CONFIGURE, NULL, "hive.exec.scratchdir", "/tmp/spark", conn);
endsendcommand(conn);
...
//			 Update Tez configuration parameters
conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.lib.uris.classpath", "{{hadoop_conf_dir}},{{hadoop_home}}/*,{{hadoop_home}}/lib/*,{{hadoop_hdfs_home}}/*,{{hadoop_hdfs_home}}/lib/*,{{hadoop_yarn_home}}/*,{{hadoop_yarn_home}}/lib/*,{{tez_home}}/*,{{tez_home}}/lib/*,{{tez_conf_dir}}", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.lib.uris", "{{tez_lib_uris}}", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.cluster.additional.classpath.prefix", "/etc/hadoop/conf/secure", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.log.level", "INFO", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.generate.debug.artifacts", "false", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.staging-dir", "/tmp/${user.name}/staging", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.resource.memory.mb", "2048", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.launch.cmd-opts", "-XX:+PrintGCDetails -verbose:gc -XX:+PrintGCTimeStamps -XX:+UseNUMA -XX:+UseG1GC -XX:+ResizeTLAB{{heap_dump_opts}}", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.launch.cluster-default.cmd-opts", "-server -Djava.net.preferIPv4Stack=true", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.launch.env", "LD_LIBRARY_PATH={{hadoop_home}}/lib/native", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.task.resource.memory.mb", "1536", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.task.launch.cmd-opts", "-XX:+PrintGCDetails -verbose:gc -XX:+PrintGCTimeStamps -XX:+UseNUMA -XX:+UseG1GC -XX:+ResizeTLAB{{heap_dump_opts}}", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.task.launch.cluster-default.cmd-opts", "-server -Djava.net.preferIPv4Stack=true", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.task.launch.env", "LD_LIBRARY_PATH={{hadoop_home}}/lib/native", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.shuffle-vertex-manager.min-src-fraction", "0.2", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.shuffle-vertex-manager.max-src-fraction", "0.4", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.am-rm.heartbeat.interval-ms.max", "250", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.grouping.split-waves", "1.7", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.grouping.min-size", "16777216", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.grouping.max-size", "1073741824", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.container.reuse.enabled", "true", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.container.reuse.rack-fallback.enabled", "true", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.container.reuse.non-local-fallback.enabled", "false", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.container.idle.release-timeout-min.millis", "10000", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.container.idle.release-timeout-max.millis", "20000", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.container.reuse.locality.delay-allocation-millis", "250", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.max.app.attempts", "2", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.maxtaskfailures.per.node", "10", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.task.am.heartbeat.counter.interval-ms.max", "4000", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.task.get-task.sleep.interval-ms.max", "200", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.task.max-events.per-heartbeat", "500", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.session.client.timeout.secs", "-1", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.session.am.dag.submit.timeout.secs", "300", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.runtime.compress", "true", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.runtime.compress.codec", "org.apache.hadoop.io.compress.SnappyCodec", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.runtime.unordered.output.buffer.size-mb", "100", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.runtime.convert.user-payload.to.history-text", "false", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.use.cluster.hadoop-libs", "false", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.tez-ui.history-url.template", "__HISTORY_URL_BASE__?viewPath=%2F%23%2Ftez-app%2F__APPLICATION_ID__", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.tez-ui.history-url.base", "", conn);  // Empty value
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.am.view-acls", "*", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.runtime.optimize.local.fetch", "true", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.task.generate.counters.per.io", "true", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.runtime.sorter.class", "PIPELINED", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.runtime.pipelined.sorter.sort.threads", "2", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.runtime.io.sort.mb", "272", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.counters.max", "10000", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.counters.max.groups", "3000", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.runtime.shuffle.fetch.buffer.percent", "0.6", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.runtime.shuffle.memory.limit.percent", "0.25", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "tez.history.logging.timeline-cache-plugin.old-num-dags-per-group", "5", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(TEZ, CONFIGURE, NULL, "yarn.timeline-service.enabled", "false", conn);
endsendcommand(conn);
stop_stdout_capture();  // Prints absolute path
				break;
 ...
 //Update Zeppelin configuration parameters
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.server.addr", "0.0.0.0", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.server.port", "9995", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.server.ssl.port", "9995", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.notebook.dir", "notebook", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.notebook.homescreen", " ", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.notebook.homescreen.hide", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.notebook.s3.user", "user", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.notebook.s3.bucket", "zeppelin", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.notebook.storage", "org.apache.zeppelin.notebook.repo.FileSystemNotebookRepo", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.config.storage.class", "org.apache.zeppelin.storage.FileSystemConfigStorage", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.config.fs.dir", "conf", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.interpreter.dir", "interpreter", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.interpreters", "org.apache.zeppelin.spark.SparkInterpreter,org.apache.zeppelin.spark.PySparkInterpreter,org.apache.zeppelin.spark.SparkSqlInterpreter,org.apache.zeppelin.spark.DepInterpreter,org.apache.zeppelin.markdown.Markdown,org.apache.zeppelin.angular.AngularInterpreter,org.apache.zeppelin.shell.ShellInterpreter,org.apache.zeppelin.jdbc.JDBCInterpreter,org.apache.zeppelin.phoenix.PhoenixInterpreter,org.apache.zeppelin.livy.LivySparkInterpreter,org.apache.zeppelin.livy.LivyPySparkInterpreter,org.apache.zeppelin.livy.LivySparkRInterpreter,org.apache.zeppelin.livy.LivySparkSQLInterpreter", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.interpreter.group.order", "spark,angular,jdbc,livy,md,sh", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.interpreter.connect.timeout", "30000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.ssl", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.ssl.client.auth", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.ssl.keystore.path", "conf/keystore", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.ssl.keystore.type", "JKS", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.ssl.keystore.password", "change me", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.ssl.key.manager.password", "change me", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.ssl.truststore.path", "conf/truststore", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.ssl.truststore.type", "JKS", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.ssl.truststore.password", "change me", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.server.allowed.origins", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.anonymous.allowed", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.notebook.public", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.websocket.max.text.message.size", "1024000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "zeppelin.interpreter.config.upgrade", "true", conn);
endsendcommand(conn);

 //Update Log4j configuration parameters
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "log4j.rootLogger", "INFO, dailyfile", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "log4j.appender.stdout", "org.apache.log4j.ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "log4j.appender.stdout.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "log4j.appender.stdout.layout.ConversionPattern", "%5p [%d{ISO8601}] ({%t} %F[%M]:%L) - %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "log4j.appender.dailyfile.DatePattern", ".yyyy-MM-dd", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "log4j.appender.dailyfile.Threshold", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "log4j.appender.dailyfile", "org.apache.log4j.DailyRollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "log4j.appender.dailyfile.File", "${zeppelin.log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "log4j.appender.dailyfile.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "log4j.appender.dailyfile.layout.ConversionPattern", "%5p [%d{ISO8601}] ({%t} %F[%M]:%L) - %m%n", conn);
endsendcommand(conn);

 //Update Shiro configuration parameters
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "users.admin", "$shiro1$SHA-256$500000$p6Be9+t2hdUXJQj2D0b1fg==$bea5JIMqcVF3J6eNZGWQ/3eeDByn5iEZDuGsEip06+M=, admin", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "users.user1", "$shiro1$SHA-256$500000$G2ymy/qmuZnGY6or4v2KfA==$v9fabqWgCNCgechtOUqAQenGDs0OSLP28q2wolPT4wU=, role1, role2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "users.user2", "$shiro1$SHA-256$500000$aHBgiuwSgAcP3Xt5mEzeFw==$KosBnN2BNKA9/KHBL0hnU/woJFl+xzJFj12NQ0fnjCU=, role3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "users.user3", "$shiro1$SHA-256$500000$nf0GzH10GbYVoxa7DOlOSw==$ov/IA5W8mRWPwvAoBjNYxg3udJK0EmrVMvFCwcr9eAs=, role2", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.passwordMatcher", "org.apache.shiro.authc.credential.PasswordMatcher", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.iniRealm.credentialsMatcher", "$passwordMatcher", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.sessionManager", "org.apache.shiro.web.session.mgt.DefaultWebSessionManager", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.cacheManager", "org.apache.shiro.cache.MemoryConstrainedCacheManager", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.securityManager.cacheManager", "$cacheManager", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.cookie", "org.apache.shiro.web.servlet.SimpleCookie", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.cookie.name", "JSESSIONID", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.cookie.httpOnly", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.sessionManager.sessionIdCookie", "$cookie", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.securityManager.sessionManager", "$sessionManager", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.securityManager.sessionManager.globalSessionTimeout", "86400000", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "main.shiro.loginUrl", "/api/login", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "roles.role1", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "roles.role2", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "roles.role3", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "roles.admin", "*", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "urls./api/version", "anon", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(ZEPPELIN, CONFIGURE, NULL, "urls./**", "authc", conn);
endsendcommand(conn);
stop_stdout_capture();
				break;
...
conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "tickTime", "3000", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "initLimit", "10", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "syncLimit", "5", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "clientPort", "2181", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "dataDir", "/hadoop/zookeeper", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "autopurge.snapRetainCount", "30", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "autopurge.purgeInterval", "24", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "4lw.commands.whitelist", "ruok", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "admin.enableServer", "true", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "admin.serverPort", "9393", conn);
endsendcommand(conn);

 //Set log rotation properties (likely in a separate config file)
conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "zookeeper_log_max_backup_size", "10", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "zookeeper_log_number_of_backup_files", "10", conn);
endsendcommand(conn);
...
endsendcommand(conn);
 ...
 //Set individual log4j configuration lines
conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.rootLogger", "INFO, CONSOLE, ROLLINGFILE", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.CONSOLE", "org.apache.log4j.ConsoleAppender", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.CONSOLE.Threshold", "INFO", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.CONSOLE.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.CONSOLE.layout.ConversionPattern", "%d{ISO8601} - %-5p [%t:%C{1}@%L] - %m%n", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.ROLLINGFILE", "org.apache.log4j.RollingFileAppender", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.ROLLINGFILE.Threshold", "DEBUG", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.ROLLINGFILE.File", "{{zk_log_dir}}/zookeeper.log", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.ROLLINGFILE.MaxFileSize", "{{zookeeper_log_max_backup_size}}MB", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.ROLLINGFILE.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.ROLLINGFILE.layout.ConversionPattern", "%d{ISO8601} - %-5p [%t:%C{1}@%L] - %m%n", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.TRACEFILE", "org.apache.log4j.FileAppender", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.TRACEFILE.Threshold", "TRACE", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.TRACEFILE.File", "zookeeper_trace.log", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.TRACEFILE.layout", "org.apache.log4j.PatternLayout", conn);
endsendcommand(conn);

conn = startcommand(host, port);
SendComponentActionCommand(ZOOKEEPER, CONFIGURE, NULL, "log4j.appender.TRACEFILE.layout.ConversionPattern", "%d{ISO8601} - %-5p [%t:%C{1}@%L][%x] - %m%n", conn);
endsendcommand(conn);
...
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "jobmanager.archive.fs.dir", "hdfs:/completed-jobs/", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "historyserver.archive.fs.dir", "hdfs:/completed-jobs/", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "historyserver.web.port", "8082", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "historyserver.archive.fs.refresh-interval", "10000", conn);
endsendcommand(conn);

 //Update security parameters
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "security.kerberos.login.keytab", "none", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "security.kerberos.login.principal", "none", conn);
endsendcommand(conn);
 //Translate template content into individual configuration parameters
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "jobmanager.rpc.address", "localhost", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "jobmanager.rpc.port", "6123", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "jobmanager.bind-host", "localhost", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "jobmanager.memory.process.size", "1024m", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "taskmanager.bind-host", "localhost", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "taskmanager.host", "localhost", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "taskmanager.memory.process.size", "1024m", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "taskmanager.numberOfTaskSlots", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "parallelism.default", "1", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "env.java.home", "{{java_home}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "env.hadoop.conf.dir", "{{hadoop_conf_dir}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "env.pid.dir", "{{flink_pid_dir}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "env.log.dir", "{{flink_log_dir}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "jobmanager.execution.failover-strategy", "region", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "rest.address", "localhost", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "rest.bind-address", "localhost", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "jobmanager.archive.fs.dir", "{{jobmanager_archive_fs_dir}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "historyserver.web.port", "{{historyserver_web_port}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "historyserver.archive.fs.dir", "{{historyserver_archive_fs_dir}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "historyserver.archive.fs.refresh-interval", "{{historyserver_archive_fs_refresh_interval}}", conn);
endsendcommand(conn);

 //Conditionally include Kerberos configuration
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "security.kerberos.login.use-ticket-cache", "{{'true' if security_enabled else ''}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "security.kerberos.login.keytab", "{{security_kerberos_login_keytab if security_enabled else ''}}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "security.kerberos.login.principal", "{{security_kerberos_login_principal if security_enabled else ''}}", conn);
endsendcommand(conn);
...
endsendcommand(conn);
 //Update Flink log4j configuration parameters
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "monitorInterval", "30", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "rootLogger.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "rootLogger.appenderRef.file.ref", "FileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.file.name", "FileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.file.type", "FILE", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.file.append", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.file.fileName", "${sys:log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.file.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.file.layout.pattern", "%d{yyyy-MM-dd HH:mm:ss,SSS} %-5p %-60c %x - %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.yarn.name", "org.apache.flink.yarn", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.yarn.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.yarn.appenderRef.console.ref", "ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.yarncli.name", "org.apache.flink.yarn.cli.FlinkYarnSessionCli", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.yarncli.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.yarncli.appenderRef.console.ref", "ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.hadoop.name", "org.apache.hadoop", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.hadoop.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.hadoop.appenderRef.console.ref", "ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.hive.name", "org.apache.hadoop.hive", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.hive.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.hive.additivity", "false", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.hive.appenderRef.file.ref", "FileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.kubernetes.name", "org.apache.flink.kubernetes", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.kubernetes.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.kubernetes.appenderRef.console.ref", "ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.console.name", "ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.console.type", "CONSOLE", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.console.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.console.layout.pattern", "%d{yyyy-MM-dd HH:mm:ss,SSS} %-5p %-60c %x - %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.hadoopnative.name", "org.apache.hadoop.util.NativeCodeLoader", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.hadoopnative.level", "OFF", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.netty.name", "org.jboss.netty.channel.DefaultChannelPipeline", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.netty.level", "OFF", conn);
endsendcommand(conn);
 //Update Flink console logging configuration
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "monitorInterval", "30", conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "rootLogger.appenderRef.console.ref", "ConsoleAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "rootLogger.appenderRef.rolling.ref", "RollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.akka.name", "akka", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.akka.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.kafka.name", "org.apache.kafka", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.kafka.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.hadoop.name", "org.apache.hadoop", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.zookeeper.name", "org.apache.zookeeper", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.zookeeper.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.shaded_zookeeper.name", "org.apache.flink.shaded.zookeeper3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.shaded_zookeeper.level", "INFO", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.console.name", "ConsoleAppender", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.name", "RollingFileAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.type", "RollingFile", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.append", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.fileName", "${sys:log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.filePattern", "${sys:log.file}.%i", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.layout.pattern", "%d{yyyy-MM-dd HH:mm:ss,SSS} %-5p %-60c %x - %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.policies.type", "Policies", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.policies.size.type", "SizeBasedTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.policies.size.size", "100MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.policies.startup.type", "OnStartupTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.strategy.type", "DefaultRolloverStrategy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.rolling.strategy.max", "${env:MAX_LOG_FILE_NUMBER:-10}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.netty.name", "org.jboss.netty.channel.DefaultChannelPipeline", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "rootLogger.appenderRef.file.ref", "MainAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.akka.name", "akka", conn);
endsendcommand(conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.name", "MainAppender", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.type", "RollingFile", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.append", "true", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.fileName", "${sys:log.file}", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.filePattern", "${sys:log.file}.%i", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.layout.type", "PatternLayout", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.layout.pattern", "%d{yyyy-MM-dd HH:mm:ss,SSS} %-5p %-60c %x - %m%n", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.policies.type", "Policies", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.policies.size.type", "SizeBasedTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.policies.size.size", "100MB", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.policies.startup.type", "OnStartupTriggeringPolicy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.strategy.type", "DefaultRolloverStrategy", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "appender.main.strategy.max", "${env:MAX_LOG_FILE_NUMBER:-10}", conn);
endsendcommand(conn);
 //Update Flink session logging configuration
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "monitorInterval", "30", conn);
...
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.zookeeper.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.shaded_zookeeper.name", "org.apache.flink.shaded.zookeeper3", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.shaded_zookeeper.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.curator.name", "org.apache.flink.shaded.org.apache.curator.framework", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.curator.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.runtimeutils.name", "org.apache.flink.runtime.util.ZooKeeperUtils", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.runtimeutils.level", "WARN", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.runtimeleader.name", "org.apache.flink.runtime.leaderretrieval.ZooKeeperLeaderRetrievalDriver", conn);
endsendcommand(conn);
conn = startcommand(host, port);
SendComponentActionCommand(FLINK, CONFIGURE, NULL, "logger.runtimeleader.level", "WARN", conn);
endsendcommand(conn);
stop_stdout_capture();
				break;
